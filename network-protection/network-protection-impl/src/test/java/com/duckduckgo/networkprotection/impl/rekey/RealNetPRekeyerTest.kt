...
class RealNetPRekeyerTest {
    ...
    @Test
    fun `doRekey - time since last rekey less than 24h - does not rekey`() = runTest {
    ...
    @Test
    fun `doRekey - registering new key fails - do not rekey`() = runTest {
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NETP_VPN)).thenReturn(true)
        whenever(wgTunnelConfig.getWgConfigCreatedAt())
    ...
    @Test
    fun `doRekey - device not locked - no rekey`() = runTest {
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NETP_VPN)).thenReturn(true)
        whenever(wgTunnelConfig.getWgConfigCreatedAt())
            .thenReturn(System.currentTimeMillis() - TimeUnit.DAYS.toMillis(1))
    ...
    @Test
    fun `forceRekey - not internal build and forced rekey - does not rekey`() = runTest {
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NETP_VPN)).thenReturn(true)
        whenever(wgTunnelConfig.getWgConfigCreatedAt())
            .thenReturn(System.currentTimeMillis() - TimeUnit.DAYS.toMillis(1))
        isDeviceLocked = true
    ...
    @Test
    fun `forceRekey - internal build and forced rekey - rekeyed`() = runTest {
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NETP_VPN)).thenReturn(true)
        whenever(wgTunnelConfig.getWgConfigCreatedAt())
            .thenReturn(System.currentTimeMillis() - TimeUnit.DAYS.toMillis(1))
        isDeviceLocked = true
    ...
    @Test
    fun `forceRekey - internal build and forced rekey but vpn disabled - no rekey`() = runTest {
    ...
    @Test
    fun `do rekey if production build - do rekey`() = runTest {
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NETP_VPN)).thenReturn(true)
        whenever(wgTunnelConfig.getWgConfigCreatedAt())
            .thenReturn(System.currentTimeMillis() - TimeUnit.DAYS.toMillis(1))
        isDeviceLocked = true
    ...
    @Test
    fun `doRekey - production build but vpn disabled - does not rekey`() = runTest {
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NETP_VPN)).thenReturn(false)
        whenever(wgTunnelConfig.getWgConfigCreatedAt())
            .thenReturn(System.currentTimeMillis() - TimeUnit.DAYS.toMillis(1))
        isDeviceLocked = true
