...
class RealWgServerApiTest {
    ...
    @Test
    fun `registerPublicKey - server selected with no server country - return selected server with null location`() = runTest {
        whenever(appBuildConfig.flavor).thenReturn(INTERNAL)
    ...
    @Test
    fun `registerInInternal - wrong server selected - returns first server`() = runTest {
    ...
    @Test
    fun `whenInternalFlavorGetWgServerDataThenStoreReturnedServers - store returned servers`() = runTest {
        whenever(appBuildConfig.flavor).thenReturn(INTERNAL)
        internalApi.registerPublicKey("testpublickey")
    ...
    @Test
    fun `registerPublicKey - user preferred country set - request for country`() = runTest {
