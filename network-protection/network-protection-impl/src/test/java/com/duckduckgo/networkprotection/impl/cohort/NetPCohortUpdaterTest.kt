...
class NetPCohortUpdaterTest {
    ...
    @Test
    fun `onVpnStarted - net pi not registered - do nothing with cohort`() = runTest {
    ...
    @Test
    fun `onVpnStarted - cohort not set - update cohort`() = runTest {
    ...
    @Test
    fun `onVpnStarted - cohort set - do nothing with cohort`() = runTest {
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
    ...
    @Test
    fun `onVpnReconfigured - netP enabled on reconfigure and cohort not yet set - update cohort`() = runTest {
        whenever(networkProtectionState.isEnabled()).thenReturn(false)
        whenever(cohortStore.cohortLocalDate).thenReturn(null)
        testee.onVpnStarted(coroutineRule.testScope)
        verifyNoInteractions(cohortStore)
    ...
    @Test
    fun `onVpnReconfigured - cohort set - do nothing with cohort`() = runTest {
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
        whenever(cohortStore.cohortLocalDate).thenReturn(LocalDate.of(2023, 1, 1))
