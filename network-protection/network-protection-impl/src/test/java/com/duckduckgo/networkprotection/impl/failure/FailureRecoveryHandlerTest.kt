...
class FailureRecoveryHandlerTest {
    ...
    @Test
    fun `onTunnelFailure - diff from handshake below threshold - do nothing`() = runTest {
    ...
    @Test
    fun `onTunnelFailure - server changed - set config and refresh NetP`() = runTest {
    ...
    @Test
    fun `onTunnelFailure - tunnel address changed - set config and refresh NetP`() = runTest {
    ...
    @Test
    fun `onTunnelFailure - create config failed - attempt max 5 times only`() = runTest {
        whenever(currentTimeProvider.getTimeInEpochSeconds()).thenReturn(1080)
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NetPVpnFeature.NETP_VPN)).thenReturn(true)
        whenever(wgTunnelConfig.getWgConfig()).thenReturn(getWgConfig(defaultServerData))
    ...
    @Test
    fun `onTunnelFailure - called twice with different tunnel address - attempt recovery once only`() = runTest {
        val newConfig = getWgConfig(updatedServerDataDifferentAddress)
        whenever(currentTimeProvider.getTimeInEpochSeconds()).thenReturn(1080)
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NetPVpnFeature.NETP_VPN)).thenReturn(true)
        whenever(wgTunnelConfig.getWgConfig()).thenReturn(getWgConfig(defaultServerData))
    ...
    @Test
    fun `onTunnelFailure - after recovery and different tunnel address - attempt recovery twice`() = runTest {
        val newConfig = getWgConfig(updatedServerDataDifferentAddress)
        whenever(currentTimeProvider.getTimeInEpochSeconds()).thenReturn(1080)
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NetPVpnFeature.NETP_VPN)).thenReturn(true)
        whenever(wgTunnelConfig.getWgConfig()).thenReturn(getWgConfig(defaultServerData))
