...
class NetPDisabledNotificationSchedulerTest {
    ...
    @Test
    fun `onVpnStopped - vpn manually stopped - do not show snooze`() = runTest {
    ...
    @Test
    fun `onVpnStopped - manually stopped with snooze but no trigger time - do not show snooze`() = runTest {
        netPSettingsLocalConfig.vpnNotificationAlerts().setEnabled(State(enable = true))
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
        whenever(networkProtectionState.isOnboarded()).thenReturn(true)
    ...
    @Test
    fun `onVpnStopped - vpn snoozed - show snooze notification`() = runTest {
        netPSettingsLocalConfig.vpnNotificationAlerts().setEnabled(State(enable = true))
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
        whenever(networkProtectionState.isOnboarded()).thenReturn(true)
