...
class NetpCohortPixelInterceptorTest {
    ...
    @Test
    fun `intercept - cohort local date not set - drop NetP pixel URL`() {
    ...
    @Test
    fun `intercept - cohort local date is set - fire netp ev pixel url`() {
    ...
    @Test
    fun `intercept - cohort local date not set and pixel url not netp - fire non-netp pixel url`() {
        whenever(netpCohortStore.cohortLocalDate).thenReturn(null)
