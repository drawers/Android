...
class RealNetworkProtectionExclusionListTest {
    ...
    @Test
    fun `isExcluded - netp enabled and app in excluded packages - return is excluded true`() = runTest {
    ...
    @Test
    fun `isExcluded - netp disabled and app in excluded packages - return is excluded false`() = runTest {
        whenever(netPExclusionListRepository.getExcludedAppPackages()).thenReturn(listOf("com.test.app"))
    ...
    @Test
    fun `isExcluded - netp enabled and app not in excluded packages - is excluded false`() = runTest {
    ...
    @Test
    fun `isExcluded - netp not enabled and app not in excluded packages - return is excluded false`() = runTest {
        whenever(netPExclusionListRepository.getExcludedAppPackages()).thenReturn(emptyList())
        whenever(vpnFeaturesRegistry.isFeatureRegistered(NetPVpnFeature.NETP_VPN)).thenReturn(false)
