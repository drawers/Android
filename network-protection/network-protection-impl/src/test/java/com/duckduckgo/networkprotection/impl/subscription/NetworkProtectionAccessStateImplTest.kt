...
class NetworkProtectionAccessStateImplTest {
    ...
    @Test
    fun `getState - subscriptions enabled and vpn inactive and netp disabled - return not unlocked`() = runTest {
    ...
    @Test
    fun `getState - subscriptions enabled and vpn signed out and netp disabled - return not unlocked`() = runTest {
    ...
    @Test
    fun `getState - subscriptions enabled, VPN inactive, NETP enabled - return not unlocked and reset VPN state`() = runTest {
        whenever(netpSubscriptionManager.getVpnStatus()).thenReturn(INACTIVE)
    ...
    @Test
    fun `getState - subscriptions enabled and vpn expired and netp enabled - return not unlocked and reset vpn state`() = runTest {
        whenever(netpSubscriptionManager.getVpnStatus()).thenReturn(EXPIRED)
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
        testee.getState().also {
            assertEquals(Locked, it)
    ...
    @Test
    fun `getStateFlow - vpn expired and netp enabled - emit not unlocked and reset vpn state`() = runTest {
        whenever(netpSubscriptionManager.vpnStatus()).thenReturn(flowOf(EXPIRED))
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
        testee.getStateFlow().test {
            assertEquals(Locked, expectMostRecentItem())
