...
class NetworkProtectionAccessStateImplTest {
    ...
    @Test
    fun `getState - subscriptions enabled and vpn expired and netp disabled - not unlocked`() = runTest {
    ...
    @Test
    fun `getState - subscriptions enabled and vpn expired and netp enabled - not unlocked and reset vpn state`() = runTest {
        whenever(netpSubscriptionManager.getVpnStatus()).thenReturn(EXPIRED)
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
        testee.getState().also {
            assertEquals(Locked, it)
    ...
    @Test
    fun `getState - subscriptions enabled and vpn signed out and netp enabled - not unlocked and reset vpn state`() = runTest {
        whenever(netpSubscriptionManager.getVpnStatus()).thenReturn(SIGNED_OUT)
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
        testee.getState().also {
            assertEquals(Locked, it)
    ...
    @Test
    fun `getStateFlow - subscriptions enabled - vpn signed out and netp disabled - not unlocked`() = runTest {
    ...
    @Test
    fun `getStateFlow - subscriptions enabled and vpn inactive and netp enabled - return flow emit not unlocked and reset vpn state`() = runTest {
        whenever(netpSubscriptionManager.vpnStatus()).thenReturn(flowOf(INACTIVE))
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
        testee.getStateFlow().test {
            assertEquals(Locked, expectMostRecentItem())
    ...
    @Test
    fun `getStateFlow - subscriptions enabled - return flow emit not unlocked and reset vpn state`() = runTest {
        whenever(netpSubscriptionManager.vpnStatus()).thenReturn(flowOf(EXPIRED))
        whenever(networkProtectionState.isEnabled()).thenReturn(true)
        testee.getStateFlow().test {
            assertEquals(Locked, expectMostRecentItem())
