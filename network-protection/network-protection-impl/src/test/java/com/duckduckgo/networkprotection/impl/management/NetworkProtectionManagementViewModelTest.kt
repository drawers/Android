...
class NetworkProtectionManagementViewModelTest {
    ...
    @Test
    fun `whenTimeDifferenceHasHoursOnly - set minutes and seconds to default`() {
        assertEquals("27:00:00", 97_200_000L.toDisplayableTimerText())
    }

    @Test
    fun `whenTimeDifferenceHasMinsOnly - set hours and seconds to default`() {
        assertEquals("00:38:00", 2_280_000L.toDisplayableTimerText())
    }

    @Test
    fun `whenTimeDifferenceHasSecondsOnly - set hours and minutes to default`() {
    ...
    @Test
    fun `onOpenSettingsFromAlwaysOnPromotionClicked - emit open vpn settings command and report pixels`() = runTest {
        testee.commands().test {
