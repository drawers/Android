...
class NetpAppExclusionListViewModelTest {
    ...
    @Test
    fun `getApps - filter is all - correct view state`() = runTest {
    ...
    @Test
    fun `getApps - filter protected only - correct view state`() = runTest {
        whenever(systemAppsExclusionRepository.getAvailableCategories()).thenReturn(emptySet())
    ...
    @Test
    fun `launchFeedback - show issue reporting - shows issue reporting page`() = runTest {
    ...
    @Test
    fun `whenUserMadeChangesToExclusionListThenOnPauseRestartVpn - on pause - restart vpn`() = runTest {
    ...
    @Test
    fun `onResume - on pause do nothing - no events expected`() = runTest {
        testee.onResume(mock())
        testee.onPause(mock())
    ...
    @Test
    fun `whenWarningShownAndCategorySetToDisabledOnSystemAppCategoryStateChanged - exclude category`() = runTest {
