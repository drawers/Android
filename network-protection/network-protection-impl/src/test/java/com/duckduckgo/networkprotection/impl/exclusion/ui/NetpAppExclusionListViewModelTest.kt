...
class NetpAppExclusionListViewModelTest {
    ...
    @Test
    fun `getApps - filter is all - return correct view state`() = runTest {
    ...
    @Test
    fun `getApps - filter is protected only - return correct viewState`() = runTest {
        whenever(systemAppsExclusionRepository.getAvailableCategories()).thenReturn(emptySet())
    ...
    @Test
    fun `getApps - filter is unprotected only - correct view state`() = runTest {
        whenever(systemAppsExclusionRepository.getAvailableCategories()).thenReturn(emptySet())
    ...
    @Test
    fun `getApps - system app in override - correct view state`() = runTest {
        whenever(systemAppsExclusionRepository.getAvailableCategories()).thenReturn(emptySet())
    ...
    @Test
    fun `getApps - system app categories available - return correct view state`() = runTest {
        whenever(systemAppOverridesProvider.getSystemAppOverridesList()).thenReturn(listOf("com.example.system"))
    ...
    @Test
    fun `onAppProtectionChanged - changed to protected - manually enable app`() {
    ...
    @Test
    fun `onAppProtectionChanged - to unprotected - show disabled dialog`() = runTest {
    ...
    @Test
    fun `onAppProtectionDisabled - no report - only manually exclude app`() = runTest {
    ...
    @Test
    fun `onAppProtectionDisabled - manually exclude and show issue reporting`() = runTest {
    ...
    @Test
    fun `onPause - changes to exclusion list - restart VPN`() = runTest {
    ...
    @Test
    fun `onPause - no changes to exclusion list - do nothing`() = runTest {
        testee.onResume(mock())
        testee.onPause(mock())
    ...
    @Test
    fun `onPause - exclusion list reset to original - do nothing`() = runTest {
        testee.onResume(mock())
        exclusionListFlow.emit(
            listOf(
                NetPManuallyExcludedApp("com.example.app1", true),
    ...
    @Test
    fun `onSystemAppCategoryStateChanged - warning shown and category set to disabled - exclude category`() = runTest {
    ...
    @Test
    fun `onSystemAppCategoryStateChanged - warning shown and category set to enabled - include category`() = runTest {
        whenever(systemAppsExclusionRepository.hasShownWarning()).thenReturn(true)
