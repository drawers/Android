...
class RealSubscriptionsManagerTest {
    ...
    @Test
    fun `recoverSubscriptionFromStore - sign in user and set token`() = runTest {
        givenUserIsNotAuthenticated()
        givenPurchaseStored()
        givenStoreLoginSucceeds()
    ...
    @Test
    fun `fetchAndStoreAllData - if token is valid - return subscription`() = runTest {
        givenUserIsAuthenticated()
        givenSubscriptionSucceedsWithEntitlements()
    ...
    @Test
    fun `fetchAndStoreAllData - if token is valid - return emit entitlements`() = runTest {
        givenUserIsAuthenticated()
        givenSubscriptionSucceedsWithEntitlements()
    ...
    @Test
    fun `purchase - user not authenticated and not purchase stored and signed in email - create account with email token`() = runTest {
    ...
    @Test
    fun `whenUserNotAuthenticatedAndPurchaseActiveInStore - recover subscription`() = runTest {
        givenUserIsNotAuthenticated()
        givenPurchaseStored()
        givenStoreLoginSucceeds()
        givenSubscriptionSucceedsWithEntitlements()
    ...
    @Test
    fun `whenValidateTokenSucceeds - billing flow uses correct external id and emits states`() = runTest {
        givenUserIsAuthenticated()
        givenSubscriptionSucceedsWithoutEntitlements(status = "Expired")
    ...
    fun `exchangeAuthToken - token fails - return throw`() = runTest {
    ...
    @Test
    fun `whenSubscribedToSubscriptionStatusAndSubscriptionExistsThenEmit - subscription status emitted`() = runTest {
        givenUserIsAuthenticated()
    ...
    @Test
    fun `whenPurchaseSuccessfulThenPurchaseCheckedAndSuccessEmit - current purchase state updated - success emitted`() = runTest {
        givenUserIsAuthenticated()
    ...
    @Test
    fun `whenPurchaseFailedThenPurchaseCheckedAndWaitingEmit - subscription status waiting - in progress and waiting`() = runTest {
        givenUserIsAuthenticated()
        givenValidateTokenFails("failure")
    ...
    @Test
    fun `whenPurchaseCanceledThenEmitCanceled - current purchase state updated`() = runTest {
        val flowTest: MutableSharedFlow<PurchaseState> = MutableSharedFlow()
        whenever(playBillingManager.purchaseState).thenReturn(flowTest)
    ...
    @Test
    fun `getAuthToken - user authenticated with subscription and token expired - return success`() = runTest {
    ...
    @Test
    fun `getAuthToken - user authenticated with subscription and token expired and entitlements exists and external id different - return failure`() = runTest {
    ...
    @Test
    fun `getAuthToken - user authenticated and token expired and purchase not valid - failure`() = runTest {
        givenUserIsAuthenticated()
        givenValidateTokenFailsAndThenSucceeds("""{ "error": "expired_token" }""")
        givenStoreLoginFails()
        givenPurchaseStored()
    ...
    @Test
    fun `signOut - subscription status unknown`() = runTest {
        givenUserIsAuthenticated()
        givenSubscriptionExists()
    ...
    @Test
    fun `whenPurchaseIsSuccessfulThenPixelIsSent - subscriptions manager current purchase state - pixel sent`() = runTest {
        givenUserIsAuthenticated()
        givenValidateTokenSucceedsWithEntitlements()
        givenConfirmPurchaseSucceeds()
    ...
    @Test
    fun `whenPurchaseFails - pixel sent`() = runTest {
        givenUserIsAuthenticated()
        givenValidateTokenFails("failure")
        givenConfirmPurchaseFails()
    ...
    @Test
    fun `deleteAccount - user authenticated and valid token - returns true`() = runTest {
        givenUserIsAuthenticated()
        givenValidateTokenSucceedsWithEntitlements()
    ...
    @Test
    fun `deleteAccount - not authenticated - returns false`() = runTest {
        givenUserIsNotAuthenticated()
