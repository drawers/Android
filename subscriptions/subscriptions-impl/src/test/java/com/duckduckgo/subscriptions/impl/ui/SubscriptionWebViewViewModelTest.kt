...
class SubscriptionWebViewViewModelTest {
    ...
    @Test
    fun `processJsCallbackMessage - subscription selected and id is in object null - failure`() = runTest {
        viewModel.currentPurchaseViewState.test {
            assertTrue(awaitItem().purchaseState is PurchaseStateView.Inactive)
    ...
    @Test
    fun `whenGetSubscriptionOptionsThenSendCommand - send command`() = runTest {
    ...
    @Test
    fun `whenGetSubscriptionsAndNoSubscriptionOfferThenSendCommandWithEmptyData - send command with empty data`() = runTest {
        privacyProFeature.allowPurchase().setEnabled(Toggle.State(enable = true))
    ...
    @Test
    fun `processJsCallbackMessage - get subscriptions and toggle off - send command with empty data`() = runTest {
    ...
    @Test
    fun `processJsCallbackMessage - feature selected and pir - command sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(EXPIRED)
        viewModel.commands().test {
            viewModel.processJsCallbackMessage(
                "test",
    ...
    @Test
    fun `processJsCallbackMessage - not in purchase flow - pixel is not sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
    ...
    @Test
    fun `start - feature selected and pir in purchase flow - pixel is sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
        whenever(subscriptionsManager.currentPurchaseState).thenReturn(flowOf(CurrentPurchase.Success))
        viewModel.start()
