...
class SubscriptionWebViewViewModelTest {
    ...
    @Test
    fun `start - purchase state changes - return correct state`() = runTest {
    ...
    @Test
    fun `processJsCallbackMessage - id in object empty - return failure`() = runTest {
        val json = """
            {"id":""}
        """.trimIndent()
    ...
    @Test
    fun `processJsCallbackMessage - subscription selected and id is null - return failure`() = runTest {
        viewModel.currentPurchaseViewState.test {
            assertTrue(awaitItem().purchaseState is PurchaseStateView.Inactive)
    ...
    @Test
    fun `processJsCallbackMessage - backToSettings - command sent`() = runTest {
        viewModel.commands().test {
    ...
    @Test
    fun `processJsCallbackMessage - backToSettingsActivateSuccess - command sent`() = runTest {
        viewModel.commands().test {
    ...
    @Test
    fun `processJsCallbackMessage - getSubscriptionOptions - send command`() = runTest {
    ...
    @Test
    fun `processJsCallbackMessage - no subscription offer - send command with empty data`() = runTest {
        privacyProFeature.allowPurchase().setEnabled(Toggle.State(enable = true))
    ...
    @Test
    fun `processJsCallbackMessage - toggle off - send command with empty data`() = runTest {
    ...
    @Test
    fun `activateSubscription - subscription active - no command sent`() = runTest {
    ...
    @Test
    fun `processJsCallbackMessage - subscription inactive - command sent`() = runTest {
    ...
    @Test
    fun `processJsCallbackMessage - expired subscription - command not sent`() = runTest {
    ...
    @Test
    fun `processJsCallbackMessage - active subscription - command not sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
        viewModel.commands().test {
            viewModel.processJsCallbackMessage("test", "setSubscription", null, null)
    ...
    @Test
    fun `processJsCallbackMessage - invalid data - command not sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(EXPIRED)
        viewModel.commands().test {
    ...
    @Test
    fun `processJsCallbackMessage - invalid feature selected - command not sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(EXPIRED)
        viewModel.commands().test {
    ...
    @Test
    fun `processJsCallbackMessage - feature is NetP - command sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(EXPIRED)
        viewModel.commands().test {
    ...
    @Test
    fun `processJsCallbackMessage - subscription selected - pixel is sent`() = runTest {
        viewModel.processJsCallbackMessage(
            featureName = "test",
    ...
    @Test
    fun `processJsCallbackMessage - in purchase flow - pixel is sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
    ...
    @Test
    fun `processJsCallbackMessage - feature selected and feature is NetP and in purchase flow - pixel is sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
        whenever(subscriptionsManager.currentPurchaseState).thenReturn(flowOf(CurrentPurchase.Success))
        viewModel.start()
    ...
    @Test
    fun `processJsCallbackMessage - feature selected and feature is NetP and not in purchase flow - pixel is not sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
    ...
    @Test
    fun `processJsCallbackMessage - feature selected and feature is ITR and in purchase flow - pixel is sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
        whenever(subscriptionsManager.currentPurchaseState).thenReturn(flowOf(CurrentPurchase.Success))
        viewModel.start()
    ...
    @Test
    fun `processJsCallbackMessage - feature selected and feature is PIR and in purchase flow - pixel is sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
        whenever(subscriptionsManager.currentPurchaseState).thenReturn(flowOf(CurrentPurchase.Success))
        viewModel.start()
    ...
    @Test
    fun `processJsCallbackMessage - feature selected and feature is PIR and not in purchase flow - pixel is not sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
    ...
    @Test
    fun `processJsCallbackMessage - subscriptions welcome FAQ clicked not in purchase flow - pixel not sent`() = runTest {
        whenever(subscriptionsManager.subscriptionStatus()).thenReturn(AUTO_RENEWABLE)
