...
class RealAuthRepositoryTest {
    ...
    @Test
    fun `isUserAuthenticated - no access token - return false`() = runTest {
        authStore.authToken = "authToken"
        assertFalse(authRepository.isUserAuthenticated())
    }

    @Test
    fun `isUserAuthenticated - no auth token - return false`() = runTest {
        authStore.accessToken = "accessToken"
        assertFalse(authRepository.isUserAuthenticated())
    }

    @Test
    fun `isUserAuthenticated - no auth token and access token - return false`() = runTest {
        assertFalse(authRepository.isUserAuthenticated())
    }

    @Test
    fun `isUserAuthenticated - authenticated - returns true`() = runTest {
        authStore.authToken = "authToken"
        authStore.accessToken = "accessToken"
    ...
    @Test
    fun `getAuthToken - returns tokens`() = runTest {
        assertNull(authStore.authToken)
        assertNull(authStore.accessToken)
    ...
    @Test
    fun `getStatus - return correct status`() = runTest {
    ...
    @Test
    fun `canSupportEncryption - support encryption false - return false`() = runTest {
