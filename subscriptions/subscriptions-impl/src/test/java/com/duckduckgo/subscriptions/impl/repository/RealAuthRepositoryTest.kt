...
class RealAuthRepositoryTest {
    ...
    @Test
    fun `isUserAuthenticated - is authenticated and no access token - returns false`() = runTest {
        authStore.authToken = "authToken"
        assertFalse(authRepository.isUserAuthenticated())
    }

    @Test
    fun `isUserAuthenticated - no auth token - false`() = runTest {
        authStore.accessToken = "accessToken"
        assertFalse(authRepository.isUserAuthenticated())
    }

    @Test
    fun `isUserAuthenticated - no auth token and access token - returns false`() = runTest {
        assertFalse(authRepository.isUserAuthenticated())
    }

    @Test
    fun `isUserAuthenticated - authenticated - true`() = runTest {
        authStore.authToken = "authToken"
        authStore.accessToken = "accessToken"
    ...
    @Test
    fun `saveAccountData - set data - set account data`() = runTest {
        assertNull(authStore.authToken)
        assertNull(authStore.externalId)
    ...
    @Test
    fun `purchaseToWaitingStatus - store waiting - waiting status`() = runTest {
        authRepository.purchaseToWaitingStatus()
        assertEquals(WAITING.statusName, authStore.status)
    }

    @Test
    fun `getStatus - various statuses returned correctly`() = runTest {
