...
class RealPlayBillingManagerTest {
    ...
    @Test
    fun `when connection failed then does not attempt loading anything - verify connect and subscription calls`() = runTest {
    ...
    @Test
    fun `when connected then returns products - subscription returned`() = runTest {
    ...
    @Test
    fun `when service not ready before launching billing flow then attempts to connect - in progress`() = runTest {
        processLifecycleOwner.currentState = RESUMED
    ...
    @Test
    fun `when connect fails with recoverable error then retry with exponential backoff - verify connect invoked`() = runTest {
