...
class RealPlayBillingManagerTest {
    ...
    @Test
    fun `processCreated - connects to billing service and loads data`() = runTest {
    ...
    @Test
    fun `when connection failed then does not attempt loading anything - verify connect and subscription calls - times zero`() = runTest {
    ...
    @Test
    fun `when connected then returns products - state - product returned`() = runTest {
    ...
    @Test
    fun `when service not ready before launching billing flow then attempts to connect - in progress - connects`() = runTest {
        processLifecycleOwner.currentState = RESUMED
    ...
    @Test
    fun `when service disconnected then attempt to connect - verify connect invoked`() = runTest {
        processLifecycleOwner.currentState = RESUMED
        billingClientAdapter.methodInvocations.clear()
    ...
    @Test
    fun `when connect fails with recoverable error then retry with exponential backoff - verify connect invoked - does not time out`() = runTest {
    ...
    @Test
    fun `launchBillingFlow - retrieves product details for provided plan id`() = runTest {
        processLifecycleOwner.currentState = RESUMED
        billingClientAdapter.launchBillingFlowResult = LaunchBillingFlowResult.Success
