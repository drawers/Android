...
class RetryTest {
    ...
    @Test
    fun `when block returns true then does not retry - retry policy satisfied`() = runTest {
    ...
    @Test
    fun `when block returns true on retry then stops retrying - retry policy stopped`() = runTest {
        val retryPolicy = RetryPolicy(
            retryCount = 10,
            initialDelay = 1.seconds,
            maxDelay = 1.seconds,
