...
class ReinstallAtbListenerTest {
    ...
    @Test
    fun `beforeAtbInit - android version 10 or lower - dont check for downloads directory`() = runTest {
    ...
    @Test
    fun `beforeAtbInit - returning user checked - don't get downloads directory`() = runTest {
    ...
    @Test
    fun `getDownloadsDirectory - directory not found - variant for returning user not set`() = runTest {
        whenever(mockAppBuildConfig.sdkInt).thenReturn(Build.VERSION_CODES.R)
        val mockDownloadsDirectory: File = mock {
