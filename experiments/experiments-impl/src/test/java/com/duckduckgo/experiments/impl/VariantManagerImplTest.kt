...
class VariantManagerImplTest {
    ...
    @Test
    fun `updateVariants - variant already persisted - variant allocator never invoked`() {
    ...
    @Test
    fun `updateVariants - variant never persisted - variant allocator never invoked`() {
        val variantsConfig = listOf(VariantConfig("variantKey", 1.0))
        whenever(mockExperimentVariantRepository.getUserVariant()).thenReturn(null)
        testee.updateVariants(variantsConfig)
    ...
    @Test
    fun `updateVariants - return user variant - no new variant allocated`() {
    ...
    @Test
    fun `updateVariants - persisted variant not found - restored to default variant`() {
        whenever(mockExperimentVariantRepository.getUserVariant()).thenReturn("variantKey")
        testee.updateVariants(emptyList())
    ...
    @Test
    fun `updateVariants - persisted variant with zero weight - variant is not restored`() {
    ...
    @Test
    fun `getVariantKey - no active variants - referrer variant returned`() {
    ...
    @Test
    fun `updateAppReferrerVariant - dataStore updated`() {
