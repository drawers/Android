...
class ExperimentFiltersManagerImplTest {
    ...
    @Test
    fun `addFilters - variant complies with locale filter - returns true`() {
    ...
    @Test
    fun `addFilters - variant complies with Android version filter - returns true`() {
    ...
    @Test
    fun `addFilters - variant complies with both filters - returns true`() {
        val locale = Locale("en", "US")
        Locale.setDefault(locale)
        whenever(mockAppBuildConfig.sdkInt).thenReturn(33)
    ...
    @Test
    fun `addFilters - locale complies and android version does not - returns false`() {
        val locale = Locale("en", "US")
        Locale.setDefault(locale)
        whenever(mockAppBuildConfig.sdkInt).thenReturn(32)
        val testEntity = addActiveVariant(localeFilter = listOf("en_US"), androidVersionFilter = listOf("33", "34"))
    ...
    @Test
    fun `addFilters - complies with android version but not locale - returns false`() {
        val locale = Locale("en", "US")
        Locale.setDefault(locale)
        whenever(mockAppBuildConfig.sdkInt).thenReturn(33)
