...
class RealAutofillSelectCredentialsListBuilderTest {
    ...
    @Test
    fun `buildFlatList - no perfect matches and multiple partial matches - first partial match added as primary button`() {
        val sortedGroup = Groups(
            perfectMatches = emptyList(),
            partialMatches = mapOf(
                Pair("foo.example.com", listOf(creds(), creds())),
    ...
    @Test
    fun `buildFlatList - perfect and multiple partial matches - list output as expected`() {
        val sortedGroup = Groups(
