...
class RealNeverSavedSiteRepositoryTest {
    ...
    @Test
    fun `addToNeverSaveList - domain added to never save list - etld plus 1 used`() = runTest {
        testee.addToNeverSaveList("foo.example.com")
        verify(secureStorage).addToNeverSaveList("example.com")
    }

    @Test
    fun `addToNeverSaveList - domain added to never save list contains query params - only etld plus 1 used`() = runTest {
