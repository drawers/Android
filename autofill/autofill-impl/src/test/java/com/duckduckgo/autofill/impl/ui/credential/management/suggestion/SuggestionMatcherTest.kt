...
class SuggestionMatcherTest {
    ...
    @Test
    fun `getDirectSuggestions - no credentials - no suggestions`() = runTest {
        configureNoShareableCredentials()
    ...
    @Test
    fun `getDirectSuggestions - credentials available but not a match - no suggestions`() = runTest {
        configureNoShareableCredentials()
    ...
    @Test
    fun `getDirectSuggestions - port differs - not a suggestion`() = runTest {
        configureNoShareableCredentials()
    ...
    @Test
    fun `getDirectSuggestions - port in visited site matches port in saved site - not a suggestion`() = runTest {
        configureNoShareableCredentials()
        val creds = listOf(creds("example.com:9000"))
