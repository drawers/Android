...
class RealEmailProtectionInContextAvailabilityRulesTest {
    ...
    @Test
    fun `configureNonEnglishLocale - not permitted`() = runTest {
    ...
    @Test
    fun `whenFeatureDisabledInRemoteConfig - not permitted`() = runTest {
        emailProtectionInContextSignupFeature.enabled = false
        assertFalse(testee.permittedToShow(ALLOWED_URL))
    }

    @Test
    fun `whenUrlOnExceptionList - not permitted`() = runTest {
