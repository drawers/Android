...
class SuggestionListBuilderTest {
    ...
    @Test
    fun `whenNoSuggestionThenEmptyListReturned - no suggestion - empty list returned`() {
        assertTrue(testee.build(emptyList(), emptyList()).isEmpty())
    }

    @Test
    fun `whenOneDirectSuggestionThenDividerAddedLast - suggestions - divider added last`() {
    ...
    @Test
    fun `whenTwoDirectSuggestionsThenDividerAddedLast - suggestions - divider added last`() {
    ...
    @Test
    fun `whenOneDirectSuggestionThenCorrectNumberOfListItemsReturned - build suggestions - correct number of list items returned`() {
        val suggestions = buildSuggestions(1)
        val list = testee.build(suggestions, emptyList())
    ...
    @Test
    fun `whenTwoDirectSuggestionsThenCorrectNumberOfListItemsReturned - correct number of list items returned`() {
        val suggestions = buildSuggestions(2)
        val list = testee.build(suggestions, emptyList())
        assertEquals(NUM_SUGGESTION_HEADERS + NUM_DIVIDERS + suggestions.size, list.size)
    }
    ...
    @Test
    fun `whenTenDirectSuggestionsThenThirteenListItemsReturned - null state - thirteen list items returned`() {
    ...
    @Test
    fun `whenDirectSuggestionAddedThenGroupNameIsCorrect - direct suggestion added - group name correct`() {
        val suggestions = buildSuggestions(1)
    ...
    @Test
    fun `whenNoDirectSuggestionsButOneShareableThenCorrectNumberOfListItemsReturned - no direct suggestions but one shareable - correct number of list items returned`() {
        val suggestions = buildSuggestions(1)
    ...
    @Test
    fun `whenNoDirectSuggestionsButMultipleShareableThenCorrectNumberOfListItemsReturned - no direct suggestions but multiple shareable - correct number of list items returned`() {
        val suggestions = buildSuggestions(10)
        val list = testee.build(emptyList(), suggestions)
        assertEquals(NUM_SUGGESTION_HEADERS + NUM_DIVIDERS + suggestions.size, list.size)
    }
    ...
    @Test
    fun `whenOneDirectAndOneShareableThenDirectSuggestionsAppearFirst - direct suggestions appear first`() {
