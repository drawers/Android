...
class CredentialListSorterTest {
    ...
    @Test
    fun `sort - list empty - returns empty list`() {
        val result = testee.sort(emptyList())
        assertTrue(result.isEmpty())
    }

    @Test
    fun `sort - case ignored - title order asserted`() {
    ...
    @Test
    fun `sort - case ignored - domain order asserted`() {
        val sorted = testee.sort(
            list.also {
    ...
    @Test
    fun `sort - domains compared - http schema ignored`() {
        val sorted = testee.sort(
            list.also {
    ...
    @Test
    fun `sort - domains compared - https schema ignored`() {
        val sorted = testee.sort(
            list.also {
    ...
    @Test
    fun `whenCombinationOfDomainsAndTitlesThenTitlesTakePreferenceWhenTheyExist - sort - titles take preference when they exist`() {
    ...
    @Test
    fun `sort - same domain without subdomain - top site sorted first`() {
        val sorted = testee.sort(
            list.also {
                it.add(credsWithDomain(id = 0, domain = "www.example.com"))
                it.add(credsWithDomain(id = 1, domain = "accounts.example.com"))
    ...
    @Test
    fun `whenDomainsStartWithDecomposableLetterThenIsDecomposedInOrdering - sort - decomposed in ordering`() {
