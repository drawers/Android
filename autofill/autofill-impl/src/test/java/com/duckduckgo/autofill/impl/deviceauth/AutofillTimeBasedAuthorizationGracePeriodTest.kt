...
class AutofillTimeBasedAuthorizationGracePeriodTest {
    ...
    @Test
    fun `isAuthRequired - no interactions - auth required`() {
        assertTrue(testee.isAuthRequired())
    }

    @Test
    fun `isAuthRequired - last successful auth before grace period - auth required`() {
    ...
    @Test
    fun `isAuthRequired - last successful auth within grace period - auth not required`() {
    ...
    @Test
    fun `isAuthRequired - last successful auth within grace period but invalidated - auth required`() {
        recordAuthorizationWithinGracePeriod()
        timeProvider.reset()
        testee.invalidate()
        assertTrue(testee.isAuthRequired())
