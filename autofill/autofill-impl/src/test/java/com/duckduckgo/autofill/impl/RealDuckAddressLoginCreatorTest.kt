...
class RealDuckAddressLoginCreatorTest {
    ...
    @Test
    fun `createLogin - autofill capabilities restrict saving - no login created`() = runTest {
    ...
    @Test
    fun `createLogin - no auto saved login id - new login saved`() = runTest {
    ...
    @Test
    fun `createLogin - auto saved login id set but no matching login found - new login saved`() = runTest {
        configureReadyToAutoSave()
        testee.createLoginForPrivateDuckAddress(DUCK_ADDRESS, TAB_ID, URL)
        verifyLoginSaved()
    }
    ...
    @Test
    fun `createLogin - auto saved login found and details already match - not saved or updated`() = runTest {
    ...
    @Test
    fun `createLogin - auto saved login found and username different - login updated`() = runTest {
    ...
    @Test
    fun `createLogin - site in never save list - does not auto save login`() = runTest {
        configureReadyToAutoSave()
    ...
    @Test
    fun `createLogin - site not in never save list - auto saves login`() = runTest {
        configureReadyToAutoSave()
        whenever(neverSavedSiteRepository.isInNeverSaveList(any())).thenReturn(false)
        testee.createLoginForPrivateDuckAddress(DUCK_ADDRESS, TAB_ID, URL)
        verifyLoginSaved()
