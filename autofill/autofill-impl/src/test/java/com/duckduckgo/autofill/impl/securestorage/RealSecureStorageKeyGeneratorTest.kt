...
class RealSecureStorageKeyGeneratorTest {
    ...
    @Test
    fun `generateKeyFromKeyMaterial - key material - algorithm should be AES`() {
        val keyMaterial = randomBytes
        assertEquals("AES", testee.generateKeyFromKeyMaterial(keyMaterial).algorithm)
    }

    @Test
    fun `generateKeyFromPassword - SDK 26 - uses derived key secret factory and algorithm is AES`() {
