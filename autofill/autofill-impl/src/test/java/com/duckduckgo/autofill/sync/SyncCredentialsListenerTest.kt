...
class SyncCredentialsListenerTest {
    ...
    @Test
    fun `onCredentialAdded - credential recently removed - cancel delete operation and do not update metadata`() = runTest {
        testee.onCredentialAdded(1)
    ...
    @Test
    fun `onCredentialRemoved - credential not reinserted - notify sync metadata`() = runTest {
        testee.onCredentialAdded(1)
        testee.onCredentialRemoved(1)
        this.advanceTimeBy(SYNC_CREDENTIALS_DELETE_DELAY + 1)
    ...
    @Test
    fun `onCredentialsAdded - multiple credentials - notify sync metadata`() {
    ...
    @Test
    fun `onCredentialsAdded - reinsert recently removed credentials - cancel delete operation and do not update metadata`() = runTest {
        testee.onCredentialsAdded(listOf(1, 2, 3, 4, 5))
    ...
    @Test
    fun `onCredentialRemoved - credentials not reinserted - notify sync metadata`() = runTest {
        testee.onCredentialsAdded(listOf(1, 2, 3, 4, 5))
