...
class RealAutofillMatchTypeDetectorTest {
    ...
    @Test
    fun `detectMatchType - subdomain match on saved site - partial match`() {
        val result = testee.detectMatchType("example.com", creds("subdomain.example.com"))
        result.assertIsPartialMatch()
    }

    @Test
    fun `detectMatchType - subdomain match on visited site - partial match`() {
    ...
    @Test
    fun `detectMatchType - no ETLD plus one match not a match - not a match`() {
