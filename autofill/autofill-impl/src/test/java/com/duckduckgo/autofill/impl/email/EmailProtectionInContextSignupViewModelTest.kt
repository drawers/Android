...
class EmailProtectionInContextSignupViewModelTest {
    ...
    @Test
    fun `whenInitialState - view state can exit without confirmation`() = runTest {
    ...
    @Test
    fun `onBackButtonPressed - webView can go back and url in allowable back list then navigate back state`() = runTest {
    ...
    @Test
    fun `onPageFinished - view state updated - can exit without confirmation`() = runTest {
    ...
    @Test
    fun `onUserDecidedNotToCancel - pixel fired`() {
    ...
    @Test
    fun `userCancelledSignup - without confirmation - pixel fired`() {
        testee.userCancelledSignupWithoutConfirmation()
        verify(pixel).fire(EMAIL_PROTECTION_IN_CONTEXT_MODAL_DISMISSED)
    }

    @Test
    fun `userCancelledSignup - view state sent - cancellation view state sent`() = runTest {
        testee.userCancelledSignupWithoutConfirmation()
        testee.viewState.test {
