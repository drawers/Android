...
class EmailProtectionInContextSignupViewModelTest {
    ...
    @Test
    fun `viewState - initial state - can exit without confirmation`() = runTest {
    ...
    @Test
    fun `viewState - initial state - can navigate backwards`() = runTest {
        testee.viewState.test {
    ...
    @Test
    fun `onBackButtonPressed - webView cannot go back - cancel in context sign up state`() = runTest {
    ...
    @Test
    fun `onBackButtonPressed - url not known - default back action taken`() = runTest {
    ...
    @Test
    fun `onBackButtonPressed - webView can go back and URL in allowable back list - navigate back state`() = runTest {
    ...
    @Test
    fun `onBackButtonPressed - back navigation consumed - state returns to viewing web content`() = runTest {
        testee.onBackButtonPressed(URL_IN_ALLOWED_BACK_NAVIGATION_LIST, true)
        testee.viewState.test {
            awaitItem() as NavigatingBack
    ...
    @Test
    fun `onPageFinished - URL in allowed back navigation list - can exit without confirmation`() = runTest {
    ...
    @Test
    fun `onPageFinished - url requires exit confirmation - confirmation required`() = runTest {
    ...
    @Test
    fun `signedInStateUpdated - url is landing page and not signed in - does not exit`() = runTest {
    ...
    @Test
    fun `onUserDecidedNotToCancelInContextSignUp - pixel fired`() {
        testee.onUserDecidedNotToCancelInContextSignUp()
        verify(pixel).fire(EMAIL_PROTECTION_IN_CONTEXT_MODAL_EXIT_EARLY_CANCEL)
    }

    @Test
    fun `onUserConfirmedCancellationOfInContextSignUp - pixel fired`() {
    ...
    @Test
    fun `userCancelledSignupWithoutConfirmation - pixel fired`() {
        testee.userCancelledSignupWithoutConfirmation()
        verify(pixel).fire(EMAIL_PROTECTION_IN_CONTEXT_MODAL_DISMISSED)
    }

    @Test
    fun `userCancelledSignupWithoutConfirmation - cancellation viewState sent`() = runTest {
        testee.userCancelledSignupWithoutConfirmation()
        testee.viewState.test {
