...
class ResultHandlerCredentialSelectionTest {
    ...
    @Test
    fun `processResult - user accepted and authenticated - correct callback invoked`() = runTest {
        configureSuccessfulAuth()
    ...
    @Test
    fun `processResult - user cancels authentication - correct callback invoked`() = runTest {
    ...
    @Test
    fun `processResult - authentication fails - correct callback invoked`() = runTest {
        configureFailedAuth()
        val bundle = bundleForUserAcceptingToAutofill("example.com")
        testee.processResult(bundle, context, "tab-id-123", Fragment(), callback)
        verify(callback).onNoCredentialsChosenForAutofill("example.com")
    ...
    @Test
    fun `processResult - missing credentials in bundle - no callback invoked`() = runTest {
        configureSuccessfulAuth()
