...
internal class CredentialsLocalWinsStrategyTest {
    ...
    @Test
    fun `processEntries - remote are more recent then local wins`() = runTest {
        givenLocalCredentials(
            twitterCredentials,
            spotifyCredentials,
        )
    ...
    @Test
    fun `processEntries - local is more recent then local wins`() = runTest {
        givenLocalCredentials(
    ...
    @Test
    fun `processEntries - local has more credentials - no changes`() = runTest {
        givenLocalCredentials(
            twitterCredentials,
            spotifyCredentials,
            amazonCredentials,
    ...
    @Test
    fun `processEntries - remote more recent and deleted then no changes`() = runTest {
        givenLocalCredentials(
            twitterCredentials,
            spotifyCredentials,
        )
    ...
    @Test
    fun `whenServerReturnsDeletedEntityThenDoNotSavecredentials - remote credentials deleted - do not save credentials`() = runTest {
        val remoteCredentials = credentialsSyncEntries(
            entries = listOf(
                twitterCredentials.toLoginCredentialEntryResponse().copy(deleted = "1"),
                spotifyCredentials.toLoginCredentialEntryResponse().copy(deleted = "1"),
