...
class EmailProtectionInContextSignUpPromptViewModelTest {
    ...
    @Test
    fun `onProtectEmailButtonPressed - commands returned - sign up result`() = runTest {
        testee.onProtectEmailButtonPressed()
    ...
    @Test
    fun `onCloseButtonPressed - pixel sent - email protection in context prompt dismissed`() {
        testee.onCloseButtonPressed()
        verify(pixel).fire(EMAIL_PROTECTION_IN_CONTEXT_PROMPT_DISMISSED)
    }

    @Test
    fun `onCloseButtonPressed - commands returned - cancel result`() = runTest {
        testee.onCloseButtonPressed()
        testee.commands.test {
    ...
    @Test
    fun `onDoNotShowAgainButtonPressed - commands returned - do not show again`() = runTest {
        testee.onDoNotShowAgainButtonPressed()
        testee.commands.test {
