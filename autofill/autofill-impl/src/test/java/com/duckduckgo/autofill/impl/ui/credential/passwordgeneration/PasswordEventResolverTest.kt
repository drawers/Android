...
class PasswordEventResolverTest {
    ...
    @Test
    fun `decideActions - no auto saved login and not autogen credentials - prompt user to save`() {
    ...
    @Test
    fun `decideActions - no auto saved login and autogen credentials - prompt to save`() {
        val autoSavedLogin = null
    ...
    @Test
    fun `decideActions - auto saved login and not autogen credentials - delete auto login and discard auto login id and prompt to save`() {
    ...
    @Test
    fun `decideActions - auto saved login and autogen credentials - update saved auto login`() {
        val autoSavedLogin = loginCreds(id = 1)
        val actions = testee.decideActions(autoSavedLogin, autogenerated = true)
