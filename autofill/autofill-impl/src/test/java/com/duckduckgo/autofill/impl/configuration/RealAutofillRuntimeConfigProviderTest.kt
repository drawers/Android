...
class RealAutofillRuntimeConfigProviderTest {
    ...
    @Test
    fun `getRuntimeConfiguration - autofill enabled - is true`() = runTest {
    ...
    @Test
    fun `getRuntimeConfiguration - no credentials for url - configuration input type credentials is false`() = runTest {
    ...
    @Test
    fun `getRuntimeConfiguration - with credentials for url - available input type credentials is true`() = runTest {
        configureAutofillCapabilities(enabled = true)
    ...
    @Test
    fun `getRuntimeConfiguration - with password only for url - username is true`() = runTest {
        val url = "example.com"
        configureAutofillCapabilities(enabled = true)
        whenever(autofillStore.getCredentials(url)).thenReturn(
            listOf(
    ...
    @Test
    fun `getRuntimeConfiguration - site in never save list - can still save credentials`() = runTest {
        val url = "example.com"
        configureAutofillEnabledWithNoSavedCredentials(url)
