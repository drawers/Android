...
class SitePermissionsRepositoryTest {
    ...
    @Test
    fun `givenPermissionNotSupported - is domain allowed to ask - not allowed`() {
        setInitialSettings()
    ...
    @Test
    fun `givenPermissionSupported - is domain allowed to ask - true`() {
        setInitialSettings()
    ...
    @Test
    fun `askForPermissionIsDisabled - domain is not allowed to ask - is not allowed`() {
    ...
    @Test
    fun `isDomainAllowedToAsk - ask for permission disabled but site permission setting is always allow - is allowed to ask`() {
    ...
    @Test
    fun `isDomainAllowedToAsk - site permission setting is deny always - not allowed to ask`() {
    ...
    @Test
    fun `isDomainAllowedToAsk - no site permission setting and drm blocked - domain is not allowed to ask`() {
    ...
    @Test
    fun `isDomainGranted - site permissions granted within 24h - return permission granted`() {
        setInitialSettings()
        val permission = PermissionRequest.RESOURCE_VIDEO_CAPTURE
        val tabId = "tabId"
    ...
    @Test
    fun `whenUserGrantsSitePermissionAlreadyInDbThenSkipSaveEntity - site permission granted - skip save entity`() = runTest {
        val testEntity = SitePermissionsEntity(domain)
        setInitialSettings(sitePermissionEntity = testEntity)
        repository.sitePermissionGranted(url, "tabId", PermissionRequest.RESOURCE_VIDEO_CAPTURE)
    ...
    @Test
    fun `sitePermissionsWebsitesFlow - site permissions websites flow`() = runTest {
    ...
    @Test
    fun `sitePermissionsAllowedFlow - site permissions allowed flow`() = runTest {
    ...
    @Test
    fun `undoDeleteAll - site permissions allowed dao - insert back to allowed`() = runTest {
        val tabId = "tabId"
        val permission = PermissionRequest.RESOURCE_AUDIO_CAPTURE
