...
class RealContentScopeScriptsTest {
    ...
    @Test
    fun `getScript - unprotected temporary changed - use new unprotected temporary value`() {
        var js = testee.getScript(null)
        verifyJsScript(js)
    ...
    @Test
    fun `isEnabled - content scope scripts enabled - return true`() {
        whenever(mockContentScopeScriptsFeature.self()).thenReturn(EnabledToggle())
        assertTrue(testee.isEnabled())
    }

    @Test
    fun `isEnabled - content scope scripts disabled - return false`() {
