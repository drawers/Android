...
class TimePassedTest {
    ...
    @Test
    fun `whenOnlyFewSecondsPassed - short formats proper time`() {
        val timePassed = TimePassed(0, 0, 25)
        assertEquals("Just Now", timePassed.shortFormat(resources))
    }

    @Test
    fun `when hours and minutes passed then short formats proper time - short format is 1 hour ago`() {
        val timePassed = TimePassed(1, 10, 0)
        assertEquals("1h ago", timePassed.shortFormat(resources))
    }

    @Test
    fun `when hours and seconds passed then short formats proper time - short format`() {
        val timePassed = TimePassed(1, 0, 30)
        assertEquals("1h ago", timePassed.shortFormat(resources))
    }
