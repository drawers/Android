...
class RealFavoritesDisplayModeSettingsRepositoryTest {
    ...
    @Test
    fun `favoritesDisplayModeFlow - current view state emitted`() = runTest {
    ...
    @Test
    fun `displayModeChanged - viewState updated`() = runTest {
        testee.favoritesDisplayModeFlow().test {
            awaitItem()
    ...
    @Test
    fun `displayModeChanged - notify syncable setting`() = runTest {
        testee.favoritesDisplayMode = UNIFIED
    ...
    @Test
    fun `getQueryFolder - sync disabled - favorites root`() = runTest {
    ...
    @Test
    fun `getQueryFolder - sync enabled and native mode - mobile root`() = runTest {
        syncStateFlow.value = SyncState.READY
    ...
    @Test
    fun `getQueryFolder - sync enabled and unified mode - mobile root`() = runTest {
        syncStateFlow.value = SyncState.READY
        savedSitesSettingsStore.favoritesDisplayMode = UNIFIED
        assertEquals(SavedSitesNames.FAVORITES_ROOT, testee.getQueryFolder())
    }
    ...
    @Test
    fun `getInsertFolder - sync disabled - favorites root`() = runTest {
        syncStateFlow.value = SyncState.OFF
        assertEquals(listOf(SavedSitesNames.FAVORITES_ROOT), testee.getInsertFolder())
    }

    @Test
    fun `getInsertFolder - sync enabled and native mode - mobile and favorites root`() = runTest {
        syncStateFlow.value = SyncState.READY
        savedSitesSettingsStore.favoritesDisplayMode = NATIVE
    ...
    @Test
    fun `getInsertFolder - sync enabled and unified mode - mobile and favorites root`() = runTest {
        syncStateFlow.value = SyncState.READY
        savedSitesSettingsStore.favoritesDisplayMode = UNIFIED
        assertTrue(testee.getInsertFolder().size == 2)
        assertTrue(testee.getInsertFolder().contains(SavedSitesNames.FAVORITES_ROOT))
    ...
    @Test
    fun `getDeleteFolder - sync disabled - delete folder is favorites root`() = runTest {
        syncStateFlow.value = SyncState.OFF
        assertEquals(listOf(SavedSitesNames.FAVORITES_ROOT), testee.getDeleteFolder("entityId"))
    }

    @Test
    fun `getDeleteFolder - sync enabled and native mode - mobile and favorites root`() = runTest {
        syncStateFlow.value = SyncState.READY
        savedSitesSettingsStore.favoritesDisplayMode = NATIVE
    ...
    @Test
    fun `getDeleteFolder - sync enabled and native mode and favorite is desktop - delete folder is mobile`() = runTest {
    ...
    @Test
    fun `getDeleteFolder - sync enabled and unified mode - all folders`() = runTest {
        syncStateFlow.value = SyncState.READY
        savedSitesSettingsStore.favoritesDisplayMode = UNIFIED
