...
class DosDetectorTest {
    ...
    @Test
    fun `isUrlGeneratingDos - more than max requests count calls with same url and delay greater than limit - returns false`() {
        runBlocking {
            for (i in 0..MAX_REQUESTS_COUNT) {
                assertFalse(testee.isUrlGeneratingDos(Uri.parse("http://example.com")))
            }
    ...
    @Test
    fun `whenMoreThanMaxRequestsCountCallsWithSameUrlAndDelayGreaterThanLimit - isUrlGeneratingDos returns false`() {
        runBlocking {
            for (i in 0..MAX_REQUESTS_COUNT) {
                assertFalse(testee.isUrlGeneratingDos(Uri.parse("http://example.com")))
            }
