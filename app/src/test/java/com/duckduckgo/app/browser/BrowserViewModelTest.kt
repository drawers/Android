...
class BrowserViewModelTest {
    ...
    @Test
    fun `onNewTabRequested - tab repository added`() = runTest {
    ...
    @Test
    fun `onOpenInNewTabRequested - tab repository added with source tab id`() = runTest {
        val url = "http://example.com"
        whenever(mockOmnibarEntryConverter.convertQueryToUrl(url)).thenReturn(url)
        whenever(mockTabRepository.liveSelectedTab).doReturn(MutableLiveData())
    ...
    @Test
    fun `whenViewStateCreated - view state created - web content should be hidden`() {
    ...
    @Test
    fun `onBookmarksActivityResult - on changed - open saved site command triggered`() {
