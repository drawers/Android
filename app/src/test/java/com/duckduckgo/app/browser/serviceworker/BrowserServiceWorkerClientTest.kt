...
class BrowserServiceWorkerClientTest {
    ...
    @Test
    fun `shouldInterceptRequest - origin header does not exist but referer exists - send to interceptor`() = runTest {
        val webResourceRequest: WebResourceRequest = mock()
    ...
    @Test
    fun `shouldInterceptRequest - no origin or referer headers - send null to interceptor`() = runTest {
        val webResourceRequest: WebResourceRequest = mock()
