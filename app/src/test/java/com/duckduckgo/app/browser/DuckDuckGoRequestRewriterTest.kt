...
class DuckDuckGoRequestRewriterTest {
    ...
    @Test
    fun `addCustomQueryParams - store contains atb - atb is added`() {
    ...
    @Test
    fun `addCustomQueryParams - store missing atb - atb is not added`() {
    ...
    @Test
    fun `shouldRewriteRequest - url is serp query - return true`() {
        val uri = "http://duckduckgo.com/?q=weather".toUri()
        assertTrue(testee.shouldRewriteRequest(uri))
    }

    @Test
    fun `shouldRewriteRequest - URL is SERP query with source and ATB - return false`() {
        val uri = "http://duckduckgo.com/?q=weather&atb=test&t=test".toUri()
        assertFalse(testee.shouldRewriteRequest(uri))
    }

    @Test
    fun `shouldRewriteRequest - DuckDuckGo static URL - return true`() {
