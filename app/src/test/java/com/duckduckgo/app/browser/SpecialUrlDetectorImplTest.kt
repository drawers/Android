...
class SpecialUrlDetectorImplTest {
    ...
    @Test
    fun `determineType - app link throws URISyntaxException - return web type`() {
    ...
    @Test
    fun `determineType - mailto URL - email type detected`() {
    ...
    @Test
    fun `determineType - url is smsto - sms type detected`() {
        val expected = Sms::class
    ...
    @Test
    fun `determineType - sms url - scheme removed`() {
        val type = testee.determineType("smsto:123-555-12323") as Sms
        assertEquals("123-555-12323", type.telephoneNumber)
    }

    @Test
    fun `determineType - custom URI scheme - non-http app link type detected`() {
        val type = testee.determineType("myapp:foo bar") as NonHttpAppLink
        assertEquals("myapp:foo bar", type.uriString)
    }

    @Test
    fun `determineType - url is not privacy pro - query type detected`() {
    ...
    @Test
    fun `determineType - url is privacy pro - privacy pro type detected`() {
    ...
    @Test
    fun `determineType - parametrized query - search query type detected`() {
        val type = testee.determineType("foo site:duckduckgo.com") as SearchQuery
        assertEquals("foo site:duckduckgo.com", type.query)
    }

    @Test
    fun `determineType - javascript scheme URL - web search type detected`() {
    ...
    @Test
    fun `determineType - URL is about scheme - web search type detected`() {
        val expected = SearchQuery::class
    ...
    @Test
    fun `determineType - file scheme URL - web search type detected`() {
        val expected = SearchQuery::class
    ...
    @Test
    fun `determineType - file scheme URL - full query retained`() {
        val type = testee.determineType("file:///sdcard/") as SearchQuery
        assertEquals("file:///sdcard/", type.query)
    }

    @Test
    fun `determineType - site scheme URL - web search type detected`() {
        val expected = SearchQuery::class
    ...
    @Test
    fun `determineType - SMS longer than max allowed - truncate to max`() {
        val longSms = randomString(SMS_MAX_LENGTH + 1)
    ...
    @Test
    fun `determineType - email content longer than max allowed - truncate to max`() {
    ...
    @Test
    fun `determineType - telephone content longer than max allowed - truncate to max`() {
    ...
    @Test
    fun `determineType - telephone prompt content longer than max allowed - truncate to max`() {
        val longTelephone = randomString(PHONE_MAX_LENGTH + 1)
    ...
    @Test
    fun `determineType - AMP link - extracted AMP link type detected`() {
    ...
    @Test
    fun `determineType - cloaked AMP link - cloaked AMP link type detected`() {
        whenever(mockAmpLinks.extractCanonicalFromAmpLink(anyString()))
    ...
    @Test
    fun `determineType - tracking parameter link - tracking parameter link type detected`() {
    ...
    @Test
    fun `determineType - url is privacy pro - privacy pro link detected`() {
        whenever(subscriptions.shouldLaunchPrivacyProForUrl(any())).thenReturn(true)
