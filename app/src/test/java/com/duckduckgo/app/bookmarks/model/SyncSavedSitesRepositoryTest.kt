...
class SyncSavedSitesRepositoryTest {
    ...
    @Test
    fun `getFolderDiff - folder metadata not present - all children in current and insert field`() = runTest {
    ...
    @Test
    fun `getFolderDiff - folder metadata not present - deleted children not present in deleted field`() = runTest {
    ...
    @Test
    fun `getFolderDiff - folder metadata present and local content has item added - contains inserted item`() = runTest {
        val entities = BookmarkTestUtils.givenSomeBookmarks(5)
        savedSitesEntitiesDao.insertList(entities)
    ...
    @Test
    fun `replaceBookmarkFolder - folder replaced - children updated`() {
    ...
    @Test
    fun `replaceBookmarkFolder - folder replaced - orphaned children removed`() {
        val bookmarks = BookmarkTestUtils.givenSomeBookmarks(10)
        savedSitesEntitiesDao.insertList(bookmarks)
    ...
    @Test
    fun `replaceBookmarkFolder - remote folder missing locally stored child - orphan is created`() {
        val bookmarks = BookmarkTestUtils.givenSomeBookmarks(10)
        savedSitesEntitiesDao.insertList(bookmarks)
    ...
    @Test
    fun `replaceBookmarkFolder - remote folder contains added children - orphans are not created`() {
        val bookmarks = BookmarkTestUtils.givenSomeBookmarks(10)
        savedSitesEntitiesDao.insertList(bookmarks)
    ...
    @Test
    fun `replaceBookmarkFolder - folder content replaced - relations are not duplicated`() {
        val remoteChildren = BookmarkTestUtils.givenSomeBookmarks(5)
    ...
    @Test
    fun `fixOrphans - does not affect favourites`() {
        // given a root folder that contains a subfolder
        savedSitesRelationsDao.insert(Relation(folderId = bookmarksRoot.entityId, entityId = folder.id))
    ...
    @Test
    fun `pruneDeleted - deleted entity - all related data is deleted`() {
        // given a root folder that contains a subfolder
        val subfolder = Entity(folder.id, "Folder", "", FOLDER, "", false)
        savedSitesEntitiesDao.insert(subfolder)
        savedSitesRelationsDao.insert(Relation(folderId = bookmarksRoot.entityId, entityId = folder.id))
    ...
    @Test
    fun `generateLocalChanges - metadata request updated`() {
    ...
    @Test
    fun `addRequestMetadata - local changes are empty - metadata request is not updated`() {
    ...
    @Test
    fun `addResponseMetadata - response has no entities - metadata is not updated`() {
        repository.addResponseMetadata(emptyList())
        assertTrue(savedSitesMetadataDao.all().isEmpty())
    }

    @Test
    fun `addResponseMetadata - no entities after patch request - metadata copied as response`() {
    ...
    @Test
    fun `addResponseMetadata - response has entities - metadata is updated`() {
    ...
    @Test
    fun `discardRequestMetadataAndStoreResponseMetadata - metadata status reflects response`() {
        val requestContent = listOf(
            getRequestEntryFromBookmarkFolder(favoritesFolder, listOf(bookmark1).map { it.id }),
            getRequestEntryFromSavedSite(bookmark1),
            getRequestEntryFromSavedSite(bookmark3),
    ...
    @Test
    fun `addResponseMetadata - response has only bookmarks - metadata is not updated`() {
        val responseEntries = listOf(
            getResponseEntryFromSavedSite(bookmark),
        )
    ...
    @Test
    fun `addResponseMetadata - no folders after patch request - metadata copied as response`() {
        givenSomeContentIn(folderId = bookmarksRootFolder.id, children = 5)
        assertTrue(savedSitesMetadataDao.get(bookmarksRootFolder.id) != null)
        assertTrue(savedSitesMetadataDao.get(bookmarksRootFolder.id)!!.childrenRequest != null)
        assertTrue(savedSitesMetadataDao.get(bookmarksRootFolder.id)!!.childrenResponse == null)
    ...
    @Test
    fun `updateModified - dates are properly updated`() {
    ...
    @Test
    fun `setLocalEntitiesForNextSync - entities present before deduplication - timestamp is updated`() {
    ...
    @Test
    fun `replaceBookmark - deduplicating bookmark - remote bookmark replaces local`() {
    ...
    @Test
    fun `replaceBookmark - different id - data is updated`() {
    ...
    @Test
    fun `replaceBookmark - same id - data is updated`() {
        val rootFolder = BookmarkFolder(id = SavedSitesNames.BOOKMARKS_ROOT, name = "root", lastModified = "timestamp", parentId = "")
        savedSitesRepository.insert(rootFolder)
    ...
    @Test
    fun `replaceBookmark - bookmark from another folder - data is updated`() {
        val rootFolder = BookmarkFolder(id = SavedSitesNames.BOOKMARKS_ROOT, name = "root", lastModified = "timestamp", parentId = "")
        val subFolder = BookmarkFolder(id = "folder", name = "Folder", lastModified = "timestamp", parentId = rootFolder.id)
        savedSitesRepository.insert(rootFolder)
        savedSitesRepository.insert(subFolder)
    ...
    @Test
    fun `replaceBookmarkFolder - move bookmark to another folder - belongs to one folder`() {
        val rootFolder = BookmarkFolder(id = SavedSitesNames.BOOKMARKS_ROOT, name = "root", lastModified = "timestamp", parentId = "")
        val subFolder = BookmarkFolder(id = "folder", name = "Folder", lastModified = "timestamp", parentId = rootFolder.id)
        savedSitesRepository.insert(rootFolder)
        savedSitesRepository.insert(subFolder)
    ...
    @Test
    fun `replaceBookmarkFolder - move favourite to another folder - belongs to favourites and folder`() {
        val rootFolder = BookmarkFolder(id = SavedSitesNames.BOOKMARKS_ROOT, name = "root", lastModified = "timestamp", parentId = "")
        val subFolder = BookmarkFolder(id = "folder", name = "Folder", lastModified = "timestamp", parentId = rootFolder.id)
        savedSitesRepository.insert(rootFolder)
        savedSitesRepository.insert(subFolder)
    ...
    @Test
    fun `getBookmarksModifiedSince - bookmark modified after threshold - has bookmarks`() {
    ...
    @Test
    fun `getBookmarksModifiedSince - modified before threshold - is empty`() {
        val since = OffsetDateTime.now(ZoneOffset.UTC).minusHours(1)
        val twoHoursAgo = OffsetDateTime.now(ZoneOffset.UTC).minusHours(2)
    ...
    @Test
    fun `replaceFavouriteFolder - replacing favourites folder - only current children are kept`() {
        // given a root folder that contains a subfolder
        savedSitesRelationsDao.insert(Relation(folderId = bookmarksRoot.entityId, entityId = folder.id))
    ...
    @Test
    fun `addToFavouriteFolder - previous favourites are kept`() {
    ...
    @Test
    fun `getInvalidSavedSites - expected saved sites returned`() = runTest {
        val bookmarks = BookmarkTestUtils.givenSomeBookmarks(10)
        savedSitesEntitiesDao.insertList(bookmarks)
