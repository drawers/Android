...
class DefaultBrowserPageViewModelTest {
    ...
    @Test
    fun `handleResult - set DDG as just once for first time - show instructions, open dialog, fire pixel`() {
        whenever(mockDefaultBrowserDetector.isDefaultBrowser()).thenReturn(false)
        testee.loadUI()
        testee.onDefaultBrowserClicked()
    ...
    @Test
    fun `handleResult - set DDG as just once max times - take user to browser`() {
        whenever(mockDefaultBrowserDetector.isDefaultBrowser()).thenReturn(false)
        val params = mapOf(
    ...
    @Test
    fun `handleResult - set DDG as default - continue to browser`() {
        val params = mapOf(
            Pixel.PixelParameter.DEFAULT_BROWSER_SET_FROM_ONBOARDING to true.toString(),
            Pixel.PixelParameter.DEFAULT_BROWSER_SET_ORIGIN to Pixel.PixelValues.DEFAULT_BROWSER_EXTERNAL,
        )
    ...
    @Test
    fun `handleResult - internal browser and DDG is default - reset times pressed just once`() {
        whenever(mockDefaultBrowserDetector.isDefaultBrowser()).thenReturn(true)
        testee.timesPressedJustOnce = 1
