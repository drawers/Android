...
class DefaultBrowserPageViewModelTest {
    ...
    @Test
    fun `whenInitializing - default browser exists - show settings UI`() {
    ...
    @Test
    fun `initializing - no default browser - show dialog UI`() {
    ...
    @Test
    fun `loadUI - show settings UI if default browser is true`() {
        whenever(mockDefaultBrowserDetector.hasDefaultBrowser()).thenReturn(true)
    ...
    @Test
    fun `onDefaultButtonClicked - default browser launched - open settings command executed`() {
        whenever(mockDefaultBrowserDetector.hasDefaultBrowser()).thenReturn(true)
        val params = mapOf(
    ...
    @Test
    fun `onDefaultBrowserClicked - set default browser from dialog - continue to browser and fire pixel`() {
        val params = mapOf(
    ...
    @Test
    fun `whenUserSetDDGAsJustOnceForFirstTime - show instructions again open dialog and fire pixel`() {
        whenever(mockDefaultBrowserDetector.isDefaultBrowser()).thenReturn(false)
        testee.loadUI()
        testee.onDefaultBrowserClicked()
    ...
    @Test
    fun `onDefaultBrowserClicked - set DDG as just once the max allowed times - take user to browser`() {
        whenever(mockDefaultBrowserDetector.isDefaultBrowser()).thenReturn(false)
        val params = mapOf(
    ...
    @Test
    fun `whenUserSetDDGAsDefault - continue to browser`() {
        val params = mapOf(
            Pixel.PixelParameter.DEFAULT_BROWSER_SET_FROM_ONBOARDING to true.toString(),
            Pixel.PixelParameter.DEFAULT_BROWSER_SET_ORIGIN to Pixel.PixelValues.DEFAULT_BROWSER_EXTERNAL,
        )
    ...
    @Test
    fun `loadUI - settings - shows default browser settings UI`() {
        whenever(mockDefaultBrowserDetector.isDefaultBrowser()).thenReturn(false)
        whenever(mockDefaultBrowserDetector.hasDefaultBrowser()).thenReturn(true)
        testee.loadUI()
        testee.onDefaultBrowserClicked()
    ...
    @Test
    fun `onDefaultBrowserClicked - fire pixel`() {
        whenever(mockDefaultBrowserDetector.hasDefaultBrowser()).thenReturn(true)
        val params = mapOf(
            Pixel.PixelParameter.DEFAULT_BROWSER_SET_FROM_ONBOARDING to true.toString(),
    ...
    @Test
    fun `onDefaultBrowserClicked - settings not selected as default - fire pixel`() {
        whenever(mockDefaultBrowserDetector.isDefaultBrowser()).thenReturn(false)
        whenever(mockDefaultBrowserDetector.hasDefaultBrowser()).thenReturn(true)
        val params = mapOf(
            Pixel.PixelParameter.DEFAULT_BROWSER_SET_ORIGIN to DEFAULT_BROWSER_SETTINGS,
    ...
    @Test
    fun `handleResult - origin received internal browser - increase times pressed just once if less than max number of attempts`() {
        whenever(mockDefaultBrowserDetector.isDefaultBrowser()).thenReturn(false)
        testee.timesPressedJustOnce = 1
