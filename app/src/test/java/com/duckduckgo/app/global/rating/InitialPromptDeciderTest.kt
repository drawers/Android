...
class InitialPromptDeciderTest {
    ...
    @Test
    fun `shouldShowPrompt - not seen prompt before and used app exact enough days - should see prompt`() = runTest {
    ...
    @Test
    fun `shouldShowPrompt - seen prompt before and not used enough - should not see prompt`() = runTest {
        whenever(mockAppDaysUsedRepository.getNumberOfDaysAppUsed()).thenReturn(NOT_ENOUGH_DAYS)
