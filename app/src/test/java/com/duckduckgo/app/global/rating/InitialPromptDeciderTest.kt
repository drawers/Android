...
class InitialPromptDeciderTest {
    ...
    @Test
    fun `shouldShowPrompt - not seen prompt before and not used app enough - should not see prompt`() = runTest {
    ...
    @Test
    fun `shouldShowPrompt - user has not seen prompt before and used the app exact enough days - should see prompt`() = runTest {
    ...
    @Test
    fun `shouldShowPrompt - user has not seen prompt and used app more than enough days - should see prompt`() = runTest {
    ...
    @Test
    fun `shouldShowPrompt - seen prompt before and not enough app usage - should not see prompt`() = runTest {
        whenever(mockAppDaysUsedRepository.getNumberOfDaysAppUsed()).thenReturn(NOT_ENOUGH_DAYS)
