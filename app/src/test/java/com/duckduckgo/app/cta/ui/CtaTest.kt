...
class CtaTest {
    ...
    @Test
    fun `pixelCancelParameters - cta is add widget auto - return empty cancel parameters`() {
        val testee = HomePanelCta.AddWidgetAuto
        assertTrue(testee.pixelCancelParameters().isEmpty())
    }
    ...
    @Test
    fun `getDaxText - trackers blocked - return sorted by prevalence`() {
        val trackers = listOf(
            TrackingEvent(
    ...
    @Test
    fun `getDaxText - trackers blocked - return only trackers with display name`() {
        val trackers = listOf(
            TrackingEvent(
                documentUrl = "facebook.com",
                trackerUrl = "facebook.com",
    ...
    @Test
    fun `getDaxText - trackers blocked - return only trackers blocked`() {
        val trackers = listOf(
            TrackingEvent(
                documentUrl = "facebook.com",
                trackerUrl = "facebook.com",
    ...
    @Test
    fun `getDaxText - multiple trackers from same network blocked - return only one with zero string`() {
        val trackers = listOf(
            TestingEntity("Facebook", "Facebook", 9.0),
            TestingEntity("Facebook", "Facebook", 9.0),
            TestingEntity("Facebook", "Facebook", 9.0),
    ...
    @Test
    fun `pixelShownParameters - try clear data CTA shown - concatenate journey stored value`() {
        val existingJourney = "s:0-t:1"
        whenever(mockOnboardingStore.onboardingDialogJourney).thenReturn(existingJourney)
        whenever(mockAppInstallStore.installTimestamp).thenReturn(System.currentTimeMillis() - TimeUnit.DAYS.toMillis(1))
