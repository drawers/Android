...
class SavedSitesSyncDataProviderTest {
    ...
    @Test
    fun `getChanges - first sync invalid bookmark - changes do not contain invalid entity`() {
    ...
    @Test
    fun `changesSince - favourites and bookmarks deleted after last sync - contain data`() {
        val lastSyncTimestamp = DatabaseDateFormatter.iso8601(twoHoursAgo)
        setLastSyncTime(lastSyncTimestamp)
    ...
    @Test
    fun `changesSince - folder moved to another folder after last sync - changes contain data`() {
    ...
    @Test
    fun `allContent - no changes need sync - children request not added to metadata`() {
        val syncChanges = parser.allContent()
        assertTrue(syncChanges.isEmpty())
