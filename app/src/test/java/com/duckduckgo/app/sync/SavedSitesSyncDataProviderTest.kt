...
class SavedSitesSyncDataProviderTest {
    ...
    @Test
    fun `whenGettingAllContentAndUsersHasFavorites - changes are not empty - not empty`() {
    ...
    @Test
    fun `getChanges - new bookmark invalid - does not contain invalid entity`() {
    ...
    @Test
    fun `whenInvalidBookmarkPresentAndValidBookmarkAddedThenOnlyValidBookmarkIncluded - repository updated - only valid bookmark included`() {
        repository.insert(invalidBookmark)
    ...
    @Test
    fun `whenNewFoldersAndBookmarksAndFavouritesSinceLastSyncThenChangesContainData - changes contain data`() {
    ...
    @Test
    fun `whenBookmarkDeletedAfterLastSyncThenChangesContainData - changes contain data`() {
        val modificationTimestamp = DatabaseDateFormatter.iso8601()
        val lastSyncTimestamp = DatabaseDateFormatter.iso8601(twoHoursAgo)
        setLastSyncTime(lastSyncTimestamp)
    ...
    @Test
    fun `whenFavouriteDeletedAfterLastSyncThenChangesContainData - changes contain data`() {
        val lastSyncTimestamp = DatabaseDateFormatter.iso8601(twoHoursAgo)
        setLastSyncTime(lastSyncTimestamp)
    ...
    @Test
    fun `whenNoChangesNeedSyncThenChildrenRequestIsNotAddedToMetadata - metadata not updated`() {
        val syncChanges = parser.allContent()
        assertTrue(syncChanges.isEmpty())
