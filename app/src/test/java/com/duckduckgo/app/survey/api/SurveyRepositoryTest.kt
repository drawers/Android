...
class SurveyRepositoryTest {
    ...
    @Test
    fun `isUserEligible - required days installed null - not eligible`() {
        val survey = Survey(
            surveyId = "id",
            status = Survey.Status.SCHEDULED,
            daysInstalled = null,
    ...
    @Test
    fun `whenRequiredDaysInstalledMinusOneThenSurveyIsEligible - survey eligible`() {
    ...
    @Test
    fun `isUserEligibleForSurvey - retention day below required days`() {
        whenever(userBrowserProperties.daysSinceInstalled()).thenReturn(0)
        val survey = Survey(
            surveyId = "id",
            status = Survey.Status.SCHEDULED,
    ...
    @Test
    fun `whenLocaleIsCanadaAndRequiredDaysInstalledMet - survey eligible`() {
    ...
    @Test
    fun `whenLocaleIsUKAndRequiredDaysInstalledMetThenSurveyIsEligible - survey eligible`() {
    ...
    @Test
    fun `whenRequiredDaysInstalledIsNullAndRetentionDaysBelowDefaultDays - return minus 1`() {
    ...
    @Test
    fun `whenRequiredDaysInstalledIsNullAndRetentionDaysAboveDefaultDays - return 0`() {
    ...
    @Test
    fun `whenRequiredDaysInstalledIsMinus1AndNotFirstDay - remaining days to survey is zero`() {
        whenever(userBrowserProperties.daysSinceInstalled()).thenReturn(4)
        val survey = Survey(
            surveyId = "id",
            status = Survey.Status.SCHEDULED,
    ...
    @Test
    fun `shouldShowSurvey - survey status done - should not show`() {
        whenever(userBrowserProperties.daysSinceInstalled()).thenReturn(4)
        val survey = Survey(
            surveyId = "id",
    ...
    @Test
    fun `givenExistingSurvey - survey exists`() {
        val survey = Survey(
            surveyId = "id",
            status = Survey.Status.SCHEDULED,
            daysInstalled = 4,
    ...
    @Test
    fun `givenNonExistingSurvey - survey does not exist`() {
