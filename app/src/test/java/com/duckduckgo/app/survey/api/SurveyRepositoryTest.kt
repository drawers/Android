...
class SurveyRepositoryTest {
    ...
    @Test
    fun `isUserEligibleForSurvey - locale not allowed - survey not eligible`() {
    ...
    @Test
    fun `remainingDaysForShowingSurvey - required days installed is minus 1 and not first day - zero`() {
        whenever(userBrowserProperties.daysSinceInstalled()).thenReturn(4)
        val survey = Survey(
            surveyId = "id",
            status = Survey.Status.SCHEDULED,
