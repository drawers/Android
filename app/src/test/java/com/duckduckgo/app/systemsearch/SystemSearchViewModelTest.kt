...
class SystemSearchViewModelTest {
    ...
    @Test
    fun `resetViewState - onboarding should not show - view not visible and unexpanded`() = runTest {
    ...
    @Test
    fun `onboardingViewState - onboarding should show - view is visible and unexpanded`() = runTest {
    ...
    @Test
    fun `userSelectedApp - app launched and pixel sent`() {
    ...
    @Test
    fun `undoDelete - quick access delete - viewState updated`() = runTest {
        val savedSite = Favorite("favorite1", "title", "http://example.com", "timestamp", 0)
        whenever(mocksavedSitesRepository.getFavorites()).thenReturn(flowOf(listOf(savedSite)))
        testee = SystemSearchViewModel(
            mockUserStageStore,
    ...
    @Test
    fun `SystemSearchViewModel - user has favorites - initial state shows favorites`() {
        val savedSite = Favorite("favorite1", "title", "http://example.com", "timestamp", 0)
        whenever(mocksavedSitesRepository.getFavorites()).thenReturn(flowOf(listOf(savedSite)))
        testee = SystemSearchViewModel(
            mockUserStageStore,
