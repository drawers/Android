...
class SystemSearchViewModelTest {
    ...
    @Test
    fun `onboarding - should not show - view is not visible and unexpanded`() = runTest {
    ...
    @Test
    fun `onboarding - should show - view is visible and unexpanded`() = runTest {
    ...
    @Test
    fun `whenOnboardingIsUnexpandedAndUserPressesToggle - onboarding expanded and pixel sent`() = runTest {
    ...
    @Test
    fun `onboarding - expanded and toggle pressed - unexpanded and pixel sent`() = runTest {
        whenOnboardingShowing()
    ...
    @Test
    fun `userRequestedClear - view state reset`() = runTest {
        testee.userUpdatedQuery(QUERY)
    ...
    @Test
    fun `userSubmitsQuery - browser launched with query and pixel sent`() {
    ...
    @Test
    fun `userSubmitsQuery - with space - browser launched with trimmed query and pixel sent`() {
        testee.userSubmittedQuery("$QUERY ")
        verify(commandObserver, atLeastOnce()).onChanged(commandCaptor.capture())
        assertEquals(Command.LaunchBrowser(QUERY), commandCaptor.lastValue)
        verify(mockPixel).fire(INTERSTITIAL_LAUNCH_BROWSER_QUERY)
    ...
    @Test
    fun `userSubmitBlankQuery - ignored`() {
    ...
    @Test
    fun `userTapsDax - onboarding completed`() = runTest {
        testee.userTappedDax()
        verify(mockUserStageStore).stageCompleted(AppStage.NEW)
    }
    ...
    @Test
    fun `onUserSelectedToUpdateQuery - edit query command sent`() {
        val query = "test"
    ...
    @Test
    fun `delete - associated bookmark deleted - repository deletes bookmark`() = runTest {
        val savedSite = Favorite("favorite1", "title", "http://example.com", "timestamp", 0)
