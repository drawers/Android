...
class AppSiteRequestDataViewStateMapperTest {
    ...
    @Test
    fun `mapFromSite - site has unknown categories - request has category is null`() {
        val site = site(
            events = listOf(
    ...
    @Test
    fun `mapFromSite - trackers from same domain different entity - contains both events`() {
        val site = site(
            events = listOf(
    ...
    @Test
    fun `mapFromSite - trackers from same domain different state - request data viewState contains both`() {
        val site = site(
            events = listOf(
                TrackingEvent("test.com", "test.com/a.js", null, MAJOR_ENTITY_A, null, BLOCKED, AD),
