...
class SyncStatsRepositoryTest {
    ...
    @Test
    fun `getYesterdayDailyStats - no attempts - daily stats empty`() {
    ...
    @Test
    fun `getYesterdayDailyStats - only past attempts and errors - daily stats has correct data`() {
    ...
    @Test
    fun `getYesterdayDailyStats - only yesterday attempts - daily stats has correct data`() {
    ...
    @Test
    fun `getYesterdayDailyStats - only today attempts - daily stats has correct data`() {
    ...
    @Test
    fun `getYesterdayDailyStats - only successful attempts - daily stats has correct data`() {
        val lastSyncTimestamp = timestamp(Instant.now().minus(1, ChronoUnit.DAYS))
        val lastSync = SyncAttempt(timestamp = lastSyncTimestamp, state = SUCCESS)
        whenever(syncStateRepository.attempts()).thenReturn(listOf(lastSync))
    ...
    @Test
    fun `getYesterdayDailyStats - only failed attempts - daily stats has correct data`() {
        val lastSyncTimestamp = timestamp(Instant.now().minus(1, ChronoUnit.DAYS))
    ...
    @Test
    fun `getYesterdayDailyStats - few attempts - daily stats has correct data`() {
        val lastSyncTimestamp = timestamp(Instant.now().minus(1, ChronoUnit.DAYS))
