...
internal class SyncEngineTest {
    ...
    @Test
    fun `triggerFeatureRead - sync disabled - no sync operation triggered`() {
    ...
    @Test
    fun `triggerSync - app opens and sync is disabled - no sync operation triggered`() {
        whenever(syncStore.isSignedIn()).thenReturn(false)
    ...
    @Test
    fun `triggerBackgroundSync - sync is disabled - no sync operation triggered`() {
        whenever(syncStore.isSignedIn()).thenReturn(false)
    ...
    @Test
    fun `triggerSync - data changes and sync is disabled - no sync operation triggered`() {
        whenever(syncStore.isSignedIn()).thenReturn(false)
    ...
    @Test
    fun `triggerSync - no local changes - nothing sent`() {
    ...
    @Test
    fun `triggerSync - account creation - state updated with success`() {
        givenLocalChanges()
        givenPatchSuccess()
    ...
    @Test
    fun `sync - syncing data disabled - no sync operation triggered`() {
        whenever(syncStore.isSignedIn()).thenReturn(true)
    ...
    fun `whenCreatingSyncAccount - data sent and state updated with error`() {
        givenLocalChanges()
        givenPatchError()
    ...
    @Test
    fun `givenAppOpenWithoutChangesAndGetRemoteSucceeds - state updated`() {
        givenNoLocalChanges()
        givenGetSuccess()
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `whenAppOpenWithoutChangesAndGetRemoteFails - state updated`() {
        givenNoLocalChanges()
        givenGetError()
    ...
    @Test
    fun `givenAppOpenWithChangesAndPatchRemoteSucceeds - state updated`() {
        givenLocalChanges()
        givenPatchSuccess()
    ...
    @Test
    fun `whenAppOpenWithChangesAndFeatureFirstSync - perform get and patch`() {
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `whenAppOpenWithChangesAndPatchRemoteFails - state updated`() {
        givenLocalChanges()
        givenPatchError()
    ...
    @Test
    fun `whenFeatureReadWithoutChangesAndGetRemoteSucceeds - state updated`() {
        givenNoLocalChanges()
        givenGetSuccess()
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `whenFeatureReadWithoutChangesAndGetRemoteFails - state updated`() {
        givenNoLocalChanges()
        givenGetError()
    ...
    @Test
    fun `triggerFeatureReadWithChangesAndPatchRemote - state updated`() {
        givenLocalChanges()
        givenPatchSuccess()
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `triggerFeatureRead - state updated with patch remote failure`() {
        givenLocalChanges()
        givenPatchError()
    ...
    @Test
    fun `whenDataChangeWithoutChangesAndGetRemoteSucceeds - state updated`() {
        givenNoLocalChanges()
        givenGetSuccess()
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `whenDataChangeWithoutChangesAndGetRemoteFails - state updated`() {
        givenNoLocalChanges()
        givenGetError()
    ...
    @Test
    fun `givenDataChangeWithChangesForFirstSync - state updated`() {
        givenFirstSyncLocalChanges()
        givenPatchSuccess()
        givenGetSuccess()
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `sync - data change with changes and patch remote fails - state updated`() {
        givenLocalChanges()
        givenPatchError()
    ...
    @Test
    fun `whenBackgroundSyncCantBeScheduled - nothing happens`() {
    ...
    @Test
    fun `triggerBackgroundSync - state updated`() {
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `triggerSync - background sync without changes - get remote fails then state is updated`() {
        whenever(syncScheduler.scheduleOperation()).thenReturn(EXECUTE)
        givenNoLocalChanges()
        givenGetError()
    ...
    @Test
    fun `sync - background sync with changes and patch remote succeeds - state updated`() {
        whenever(syncScheduler.scheduleOperation()).thenReturn(EXECUTE)
        givenLocalChanges()
        givenPatchSuccess()
    ...
    @Test
    fun `triggerSync - first sync background sync with changes and patch remote succeeds - state updated`() {
        whenever(syncScheduler.scheduleOperation()).thenReturn(EXECUTE)
        givenFirstSyncLocalChanges()
        givenPatchSuccess()
        givenGetSuccess()
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `sync - background sync with changes and patch remote fails - state updated`() {
        whenever(syncScheduler.scheduleOperation()).thenReturn(EXECUTE)
        givenLocalChanges()
        givenPatchError()
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `whenAccountLoginGetRemoteFails - state updated`() {
        givenLocalChanges()
        givenGetError()
    ...
    @Test
    fun `whenAccountLoginSucceeds - state updated`() {
        givenFirstSyncLocalChanges()
        givenPatchSuccess()
        givenGetSuccess()
    ...
    @Test
    fun `triggerSync - in progress - dismissed`() {
    ...
    @Test
    fun `sync - patch new data fails because count limit - notify feature`() {
        givenLocalChanges()
    ...
    @Test
    fun `sync - patch new data fails because content too large - notify feature`() {
        givenLocalChanges()
    ...
    @Test
    fun `sync - patch new data fails because non-feature error - do not notify feature`() {
        givenLocalChanges()
        givenPatchError()
        val persisterPluginMock = mock<SyncableDataPersister>()
        whenever(persisterPlugins.getPlugins()).thenReturn(listOf(persisterPluginMock))
    ...
    @Test
    fun `syncTriggered - daily pixel sent`() {
        givenLocalChanges()
        givenPatchSuccess()
    ...
    @Test
    fun `sync - triggered with changes and patch remote succeeds with timestamp conflict - state updated and pixel fired`() {
    ...
    @Test
    fun `sync - state updated and pixel fired`() {
