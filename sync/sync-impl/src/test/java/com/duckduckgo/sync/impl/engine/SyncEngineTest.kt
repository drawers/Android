...
internal class SyncEngineTest {
    ...
    @Ignore
    // https://app.asana.com/0/1204842202586359/1205158805627400/f
    fun `triggerSync - app open with changes and patch remote fails - state is updated`() {
        givenLocalChanges()
        givenPatchError()
    ...
    @Test
    fun `triggerSync - patch new data fails because content too large - notify feature`() {
        givenLocalChanges()
    ...
    @Test
    fun `triggerSync - patch remote succeeds with orphans - state updated and pixel fired`() {
