...
class SyncRemoteFeatureToggleTest {
    ...
    @Test
    fun `showSync - feature disabled - internal build shows sync`() {
    ...
    @Test
    fun `showSync - feature disabled - false`() {
    ...
    @Test
    fun `allowDataSyncing - show sync disabled - all features disabled`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
            showSync = false
        }
        givenSyncRemoteFeatureToggle(syncFeature)
    ...
    @Test
    fun `allowDataSyncing - allowDataSyncing false`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
    ...
    @Test
    fun `allowDataSyncing - enabled but not for this version - allow data syncing on newer version true`() {
    ...
    @Test
    fun `allowSetupFlows - data syncing false - setup flows and create account disabled`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
            allowDataSyncing = false
        }
        givenSyncRemoteFeatureToggle(syncFeature)
    ...
    @Test
    fun `allowDataSyncing - allow setup flows false - enabled`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
    ...
    @Test
    fun `allowSetupFlows - setup flows false - setup flows and create account disabled`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
            allowSetupFlows = false
        }
        givenSyncRemoteFeatureToggle(syncFeature)
    ...
    @Test
    fun `allowCreateAccount - allowCreateAccount false - false`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
    ...
    @Test
    fun `allowCreateAccount - not for this version - allowCreateAccountOnNewerVersion true`() {
        whenever(appBuildConfig.versionCode).thenReturn(1)
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
    ...
    @Test
    fun `allowDataSyncing and allowSetupFlows - allowCreateAccount false - enabled`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
            allowCreateAccount = false
        }
        givenSyncRemoteFeatureToggle(syncFeature)
