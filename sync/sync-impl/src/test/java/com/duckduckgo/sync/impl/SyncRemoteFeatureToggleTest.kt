...
class SyncRemoteFeatureToggleTest {
    ...
    @Test
    fun `whenFeatureDisabledThenInternalBuildShowSyncTrue - internal build - show sync true`() {
    ...
    @Test
    fun `whenFeatureDisabledThenShowSyncIsFalse - show sync is false`() {
    ...
    @Test
    fun `allowDataSyncing - false - false`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
    ...
    @Test
    fun `allowSetupFlows - not for this version - allow setup flows on newer version true`() {
        whenever(appBuildConfig.versionCode).thenReturn(1)
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
    ...
    @Test
    fun `allowSetupFlows - setup flows and create account disabled`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
            allowSetupFlows = false
        }
        givenSyncRemoteFeatureToggle(syncFeature)
    ...
    @Test
    fun `whenAllowCreateAccountFalseThenDataSyncingAndSetupFlowsEnabled - data syncing and setup flows enabled`() {
        val syncFeature = TestSyncFeature(appBuildConfig).apply {
            allowCreateAccount = false
        }
        givenSyncRemoteFeatureToggle(syncFeature)
