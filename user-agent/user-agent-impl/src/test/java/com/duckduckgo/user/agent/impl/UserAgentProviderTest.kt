...
class UserAgentProviderTest {
    ...
    @Test
    fun `whenMobileUaRetrieved - device stripped and application component added before safari - matches expected regex`() {
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
    ...
    @Test
    fun `whenDesktopUaRetrieved - device stripped and application component added before safari - matches desktop regex`() {
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
    ...
    @Test
    fun `whenMissingVersionComponentThenUaContainsMozillaAndApplicationAndSafariComponents - ua contains mozilla and application and safari components`() {
    ...
    @Test
    fun `getUa - domain has exception - default ua`() {
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
    ...
    @Test
    fun `getUa - subdomain has exception - default ua`() {
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
    ...
    @Test
    fun `getUa - domain supports version - includes version component in usual location`() {
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
        val actual = testee.userAgent(DOMAIN)
        assertTrue("$actual does not match expected regex", ValidationRegex.converted.matches(actual))
    }
    ...
    @Test
    fun `getUserAgent - desktop user agent for specific site - returns desktop user agent`() {
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
    ...
    @Test
    fun `whenUserAgentShouldBeDefaultAndShouldUseDesktopAgentThenReturnDesktopUserAgent - return desktop user agent`() {
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
    ...
    @Test
    fun `whenUserAgentAndUrlAllowedByUserThenReturnDefaultUserAgent - return default user agent`() {
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
    ...
    @Test
    fun `getUserAgent - user agent and url allowed by user and is desktop - returns default desktop user agent`() {
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
    ...
    @Test
    fun `getDesktopUserAgent - is desktop and aarch64 - return x86_64 desktop user agent`() {
        whenever(userAgent.useLegacyUserAgent(anyString())).thenReturn(false)
        testee = getUserAgentProvider(Agent.DEFAULT, deviceInfo)
        val actual = testee.userAgent(DOMAIN, true)
