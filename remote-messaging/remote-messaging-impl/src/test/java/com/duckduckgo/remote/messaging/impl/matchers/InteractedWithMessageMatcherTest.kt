...
class InteractedWithMessageMatcherTest {
    ...
    @Test
    fun `map - interacted with message - return matching attribute`() = runTest {
    ...
    @Test
    fun `map - json matching attribute value is not list - return null`() = runTest {
        val matcher = InteractedWithMessageMatcher(remoteMessagingRepository)
    ...
    @Test
    fun `evaluate - dismissed message id matches - return true`() = runTest {
    ...
    @Test
    fun `evaluate - one dismissed message id matches - return true`() = runTest {
        givenMessageIdDismissed(listOf("1", "2", "3"))
        val matcher = InteractedWithMessageMatcher(remoteMessagingRepository)
    ...
    @Test
    fun `evaluate - no dismissed message id matches - return false`() = runTest {
        givenMessageIdDismissed(listOf("1", "2", "3"))
        val matcher = InteractedWithMessageMatcher(remoteMessagingRepository)
    ...
    fun `evaluate - empty list in matching attribute - throws exception`() = runTest {
        givenMessageIdDismissed(listOf("1", "2", "3"))
        val matcher = InteractedWithMessageMatcher(remoteMessagingRepository)
