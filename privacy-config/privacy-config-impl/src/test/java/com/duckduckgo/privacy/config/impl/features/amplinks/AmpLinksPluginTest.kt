...
class AmpLinksPluginTest {
    ...
    @Test
    fun `store - feature name does not match amp links - returns false`() {
    ...
    @Test
    fun `store - feature name matches amp links - returns true`() {
    ...
    @Test
    fun `store - feature name matches amp links and has min supported version - stores min supported version`() {
    ...
    @Test
    fun `whenFeatureNameMatchesAmpLinks - update all existing values`() {
        val jsonString = FileUtilities.loadText(AmpLinksPluginTest::class.java.classLoader!!, "json/amp_links.json")
