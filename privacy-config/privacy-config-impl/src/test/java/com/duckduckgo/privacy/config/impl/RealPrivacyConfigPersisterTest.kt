...
class RealPrivacyConfigPersisterTest {
    ...
    @Test
    fun `persistPrivacyConfig - version is lower than previous one - do nothing`() =
        runTest {
    ...
    @Test
    fun `persistPrivacyConfig - version is higher - store new config`() =
        runTest {
    ...
    @Test
    fun `persistPrivacyConfig - version higher and different plugins - store new config`() =
        runTest {
            sharedPreferences.edit().putInt("plugin_signature", 0)
            privacyRepository.insert(PrivacyConfig(version = 1, readme = "readme", eTag = "eTag", timestamp = "2023-01-02"))
