...
class RealGpcTest {
    ...
    @Test
    fun `canUrlAddHeaders - feature not enabled, Gpc enabled, URL in consumers list, header does not exist - return false`() {
        givenFeatureIsNotEnabledButGpcIsEnabled()
    ...
    @Test
    fun `canGpcBeUsedByUrl - feature and Gpc enabled and url not an exception - return true`() {
        givenFeatureAndGpcAreEnabled()
    ...
    @Test
    fun `canGpcBeUsedByUrl - feature enabled but Gpc not enabled and url not exception - return false`() {
        givenFeatureIsEnabledButGpcIsNot()
