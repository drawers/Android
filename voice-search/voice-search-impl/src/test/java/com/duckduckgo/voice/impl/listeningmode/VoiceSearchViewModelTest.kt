...
class VoiceSearchViewModelTest {
    ...
    @Test
    fun `start - start speech recognizer`() {
    ...
    @Test
    fun `start - view state initial - empty`() = runTest {
        testee.startVoiceSearch()
    ...
    @Test
    fun `whenVolumeUpdateReceived - emit update voice indicator command`() = runTest {
        val captor = argumentCaptor<(Event) -> Unit>()
        testee.startVoiceSearch()
        verify(speechRecognizer).start(captor.capture())
    ...
    @Test
    fun `startVoiceSearch - view state restarted - emits view state with no unsent result`() = runTest {
        val captor = argumentCaptor<(Event) -> Unit>()
        testee.startVoiceSearch()
        verify(speechRecognizer).start(captor.capture())
        testee.startVoiceSearch()
    ...
    @Test
    fun `startVoiceSearch - view state updated - emits unsent result`() = runTest {
        val captor = argumentCaptor<(Event) -> Unit>()
        testee.startVoiceSearch()
        verify(speechRecognizer).start(captor.capture())
        captor.firstValue.invoke(Event.PartialResultReceived("First"))
    ...
    @Test
    fun `startVoiceSearch - view model restarted - emits view state with appended result and unsent result`() = runTest {
        val captor = argumentCaptor<(Event) -> Unit>()
        testee.startVoiceSearch()
        verify(speechRecognizer).start(captor.capture())
        captor.firstValue.invoke(Event.PartialResultReceived("First"))
