...
class IdeFunctionNameDetector : TestFunctionNameDetector() {
    ...
    private fun getLintFix(
        ...
        if (Scope.ALL_JAVA_FILES !in context.scope) return null

        val response = model.generate(prompt, UserMessage.from(method.sourcePsi!!.text)).content().text()
        context.log(null, response)
        context.thisLogger().warn("Response from OpenAI: ")
        context.thisLogger().warn(response)
    ...
    companion object {
        ...
        val model: ChatLanguageModel by lazy(SYNCHRONIZED) {
            OpenAiChatModel.builder()
                .apiKey(System.getProperty("OPEN_AI_API_KEY"))
