...
class AppTrackerBlockingStatsRepositoryTest {
    ...
    @Test
    fun `getVpnTrackers - single tracker entry added but no related entity - return no tracker`() = runBlocking {
    ...
    @Test
    fun `getVpnTrackers - same tracker found multiple times - all instances returned`() = runBlocking {
        val trackerDomain = "example.com"
        addTrackerAndReturn(trackerDomain)
        val entity = addTrackerAndReturn(trackerDomain).asEntity()
