...
class DeviceShieldTrackerActivityViewModelTest {
    ...
    @Test
    fun `onViewEvent - launch app trackers - command is launch app trackers`() = runBlocking {
    ...
    @Test
    fun `onViewEvent - launch excluded apps - command is launch manage apps protection`() = runBlocking {
        viewModel.commands().test {
    ...
    @Test
    fun `onVPNPermissionNeeded - vpn permission result denied and request time smaller than needed - show vpn always on conflict dialog`() = runBlocking {
        viewModel.commands().test {
            val permissionIntent = Intent()
            viewModel.onVPNPermissionNeeded(permissionIntent)
            assertEquals(DeviceShieldTrackerActivityViewModel.Command.RequestVPNPermission(permissionIntent), awaitItem())
    ...
    @Test
    fun `onViewEvent - show remove feature confirmation dialog`() = runBlocking {
        viewModel.commands().test {
    ...
    @Test
    fun `onViewEvent - remove feature accepted - vpn and screen closed`() = runBlocking {
        viewModel.commands().test {
    ...
    @Test
    fun `onViewEvent - promote always on open settings selected - command sent`() = runBlocking {
        viewModel.commands().test {
