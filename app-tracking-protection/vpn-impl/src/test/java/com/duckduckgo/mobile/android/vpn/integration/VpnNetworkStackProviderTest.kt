...
class VpnNetworkStackProviderTest {
    ...
    @Test
    fun `registerAppTp - provider network stack and app tp registered - returns network stack`() = runTest {
    ...
    @Test
    fun `provideNetworkStack - provider network stack and features contain app tp - returns network stack`() = runTest {
        vpnFeaturesRegistry.registerFeature(VpnFeature { "unknown" })
        vpnFeaturesRegistry.registerFeature(AppTpVpnFeature.APPTP_VPN)
