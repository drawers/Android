...
class DeviceShieldActivityFeedViewModelTest {
    ...
    @Test
    fun `getMostRecentTrackers - not empty - start with skeleton then emit`() = runBlocking {
    ...
    @Test
    fun `getMostRecentTrackers - not empty and outside time window - emit loading skeleton`() = runBlocking {
    ...
    @Test
    fun `getMostRecentTrackers - vpn enabled and bounded config - emit description and apps protection state`() = runBlocking {
        mockVpnEnabled()
    ...
    @Test
    fun `getMostRecentTrackers - empty and VPN disabled with five rows - emit description`() = runBlocking {
        mockVpnDisabled()
        whenever(mockExcludedApps.getAppsAndProtectionInfo()).thenReturn(flowOf(listOf(app, excludedApp)))
        val boundedConfig = config.copy(maxRows = 5)
    ...
    @Test
    fun `getMostRecentTrackers - less trackers than max rows and VPN enabled - emit only tracker feed data items`() = runBlocking {
        repository.insert(listOf(dummyTrackers[0], dummyTrackers[1], dummyTrackers[2]))
