...
class VpnReminderNotificationContentPluginKtTest {
    @Test
    fun `getHighestPriorityPluginForType - empty plugin point - return null`() {
        assertNull(createPluginPoint(emptyList()).getHighestPriorityPluginForType(REVOKED))
    }

    @Test
    fun `getHighestPriorityPluginForType - plugin for type available - return plugin`() {
    ...
    @Test
    fun `getHighestPriorityPluginForType - multiple plugins for type - return highest priority`() {
        val plugin = createPluginPoint(
            listOf(
                FakeVpnReminderNotificationContentPlugin(type = DISABLED, NORMAL),
