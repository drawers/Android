...
class RealCohortStoreTest {
    ...
    @Test
    fun `whenInitialCohortFirstCalled - store initial cohort - stored local date`() = runTest {
    ...
    @Test
    fun `registerFeature - subsequent cohort called - no-op`() = runTest {
        vpnFeaturesRegistry.registerFeature(AppTpVpnFeature.APPTP_VPN)
        val date = LocalDate.now().plusDays(3)
        cohortStore.setCohortLocalDate(date)
    ...
    @Test
    fun `getCohort - app tp not registered - do nothing with cohort`() {
        (cohortStore as RealCohortStore).onVpnStarted(TestScope())
    ...
    @Test
    fun `whenVpnReconfiguredCalled - store initial cohort - cohort stored`() = runTest {
        vpnFeaturesRegistry.registerFeature(AppTpVpnFeature.APPTP_VPN)
