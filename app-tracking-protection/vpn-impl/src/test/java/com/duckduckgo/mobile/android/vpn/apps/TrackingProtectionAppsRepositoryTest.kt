...
class TrackingProtectionAppsRepositoryTest {
    ...
    @Test
    fun `getAppProtectionStatus - enabled app but excluded in NetP - unprotected through NetP`() = runTest {
        whenever(packageManager.getApplicationInfo("com.example.app1", 0))
            .thenReturn(ApplicationInfo().apply { packageName = "com.example.app1" })
    ...
    @Test
    fun `getAppProtectionStatus - called on ddg app - return unprotected`() = runTest {
