...
class AppTPReminderNotificationSchedulerTest {
    ...
    @Test
    fun `onVpnStopped - VPN stopped but appTP disabled was removed - nothing is enqueued`() = runTest {
        whenever(vpnFeatureRemover.isFeatureRemoved()).thenReturn(false)
        whenever(appTrackingProtection.isEnabled()).thenReturn(false)
        whenever(appTrackingProtection.isOnboarded()).thenReturn(true)
    ...
    @Test
    fun `onVpnStopped - daily reminder was enqueued - still enqueued`() = runTest {
        whenever(vpnFeatureRemover.isFeatureRemoved()).thenReturn(false)
        enqueueDailyReminderNotificationWorker()
        assertWorkersAreEnqueued(VpnReminderNotificationWorker.WORKER_VPN_REMINDER_DAILY_TAG)
    ...
    @Test
    fun `onVpnStopped - vpn manually stops - undesired reminder not scheduled`() = runTest {
        whenever(vpnFeatureRemover.isFeatureRemoved()).thenReturn(false)
        testee.onVpnStopped(coroutinesTestRule.testScope, SELF_STOP())
    ...
    @Test
    fun `onVpnStopped - user not onboarded and VPN manually stops with content plugin disabled - no immediate notification shown`() = runTest {
        whenever(mockPluginPoint.getPlugins()).thenReturn(listOf(fakeRevokedPlugin, fakeDisabledPlugin))
        whenever(vpnFeatureRemover.isFeatureRemoved()).thenReturn(false)
        whenever(appTrackingProtection.isEnabled()).thenReturn(true)
    ...
    @Test
    fun `onVpnStopped - vpn revoked and no content plugin - no immediate notification shown`() = runTest {
        whenever(vpnFeatureRemover.isFeatureRemoved()).thenReturn(false)
    ...
    @Test
    fun `onVpnStopped - appTP enabled and onboarded, VPN revoked with content plugin - immediate notification shown`() = runTest {
        whenever(mockPluginPoint.getPlugins()).thenReturn(listOf(fakeRevokedPlugin, fakeDisabledPlugin))
        whenever(appTrackingProtection.isEnabled()).thenReturn(true)
        whenever(appTrackingProtection.isOnboarded()).thenReturn(true)
    ...
    @Test
    fun `onVpnStopped - appTP disabled and VPN revoked with content plugin for revoked - no immediate notification`() = runTest {
        whenever(mockPluginPoint.getPlugins()).thenReturn(listOf(fakeRevokedPlugin, fakeDisabledPlugin))
        whenever(appTrackingProtection.isEnabled()).thenReturn(false)
        whenever(appTrackingProtection.isOnboarded()).thenReturn(true)
    ...
    @Test
    fun `onVpnStopped - appTP not onboarded and VPN revoked - no immediate notification`() = runTest {
        whenever(mockPluginPoint.getPlugins()).thenReturn(listOf(fakeRevokedPlugin, fakeDisabledPlugin))
        whenever(appTrackingProtection.isEnabled()).thenReturn(false)
        whenever(appTrackingProtection.isOnboarded()).thenReturn(false)
    ...
    @Test
    fun `onVpnStopped - reminder was scheduled - undesired reminder is no longer scheduled`() = runTest {
        enqueueUndesiredReminderNotificationWorker()
        assertWorkersAreEnqueued(VpnReminderNotificationWorker.WORKER_VPN_REMINDER_UNDESIRED_TAG)
        whenever(appTrackingProtection.isOnboarded()).thenReturn(true)
        whenever(appTrackingProtection.isEnabled()).thenReturn(true)
